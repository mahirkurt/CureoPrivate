# Output Template — Global Sözel Kimlik Raporu (v2.1)

> **Mandatory load.** Bu dosya, her `brand-maker` çağrısının üreteceği **kanonik çıktı formatını** içerir. Format bağlayıcıdır. v2.0 itibariyle 7-bölümlü yapı (v1.2'de 6 idi — Section 5 Post-Digital Readiness Scorecard eklendi).

> **v2.1 — Çıktı Sözleşmesi Dürüstlüğü (uzman-denetim düzeltmesi F).** Denetim, raporun bir alan adını **tek sinyalle "müsait"** ilan ettiğini (auronza.com/nortanza.com aslında DOLU) ve pharma marka çakışmasını **tek yüzeysel aramayla "temiz"** dediğini bulguladı. v2.1 ile rapor **doğrulama-durumu, kaynak ve zaman damgası** taşımak zorundadır; **hiçbir alan adı iki uyuşan canlı kaynak olmadan "müsait" YAZILMAZ**; **naif-algı (Layer 6)** ve **çeşitlilik-kapısı** sonuçları rapora **ZORUNLU** girer. Aşağıdaki üç ayrım her raporda korunur:
> - **doğrulama-durumu**: `confirmed` (2 kaynak) · `provisional` (tek kaynak) · `unverified` (kaynak erişilemedi) — asla varsayımsal "müsait".
> - **ön-eleme (YAPILDI)** ≠ **resmî araştırma (YAPILMADI)**: skill yalnız ön-tarama yapar; "temiz/müsait" kesin hüküm değildir.
> - **niyet ≠ algı**: her finalist için naif ilk-okuma raporlanır (Ortanza→"orta").

---

## Format İskeleti

Aşağıdaki iskelet **olduğu gibi** kullanılır. Süslü emoji veya gereksiz dekorasyon eklemeyin.

```markdown
# GLOBAL SÖZEL KİMLİK RAPORU
## [Marka / Ürün Adı: Brief'in başlığı]

**Hazırlayan**: brand-maker v2.0
**Müşteri brief'i tarihi**: [bugünün tarihi]
**Hedef pazar**: [coğrafi & demografik özet — 1 satır]
**Stratejik kategori**: [B2B SaaS / DTC / Pharma / Fintech / Web3 / vd.]
**Pharma-regulatory status**: [Applicable | N/A]

---

## 1. Stratejik Deşifre

### 1.1 Kategori Manzarası
[3-5 cümle: bu marka hangi global kategoride yarışıyor? En büyük rakipler kim?]

### 1.2 Rakip Kelime Haritası
| Rakip | Sahiplenilen Kelime | Sahiplenme Mekanizması |
|---|---|---|
| [Rakip A] | [tek kelime] | [tek cümle] |
| [Rakip B] | [tek kelime] | [tek cümle] |
| [Rakip C] | [tek kelime] | [tek cümle] |

### 1.3 Beyaz Alan Tespiti
[2-4 cümle: rakip kelime havuzunda hangi evrensel kavram boş?]

### 1.4 Stratejik Çıpa
**Önerilecek tüm isimlerin sahiplenmeye çalışacağı tek kelime / tek duygu**:
> **[ÇIPA KELIMESI]** — [tek cümle gerekçe]

### 1.5 [v2.0] Purpose-Axis Coordinate (purpose-driven brief için)
**Utility × Heritage × Rebellion × Craft** dört-eksende marka koordinatı:
> **[Eksen kombinasyonu]** — [gerekçe]

---

## 2. Beş-Kategori Bazlı Aday Havuzu (v2.0)

Aşağıdaki 10–15 aday, beş stratejik kategoriden üretilmiştir. Her kategoride 2–3 aday.

### A. Tanımlayıcı & Metaforik
1. **[Aday 1]** — [tek satır açıklama]
2. **[Aday 2]** — [tek satır açıklama]

### B. Sentez & Türetilmiş (Portmanteau)
1. **[Aday 3]** — [kök1 + kök2 = ...]
2. **[Aday 4]** — [kök1 + kök2 = ...]

### C. Soyut & Fonetik (Coined)
1. **[Aday 5]** — [fonetik mantık]
2. **[Aday 6]** — [fonetik mantık]

### D. Çağrışımsal & Mitolojik
1. **[Aday 7]** — [referans kaynağı + modern okuma]
2. **[Aday 8]** — [referans kaynağı + modern okuma]

### E. [v2.0] Sezgisel / Kontraryen
1. **[Aday 9]** — [konvansiyon + anti-thesis mantığı]
2. **[Aday 10]** — [konvansiyon + anti-thesis mantığı]

### 2.1 [v2.1] Çeşitlilik Kapısı (Shortlist Diversity Gate) — ZORUNLU

> **Bağlayıcı kural (v2.1):** Aday/finalist KÜMESİ, finalist analizinden ÖNCE `scripts/shortlist_diversity_check.py` ile taranır. Kapı, kümenin **tek bir morfem/kafiye ailesinin varyasyonları** (denetimde 14 adayın tümü `-onta/-anta/-anza/-venta/-vanza`) olup olmadığını ölçer. Kapı **FAIL** verirse, kısa liste **BLOKLANIR** ve **ikinci üretim turu** farklı fonetik/morfolojik ailelerden (Adım 3.2/3.3) TETİKLENİR.

| Metrik | Değer | Eşik | Durum |
|---|---|---|---|
| En büyük terminal-kafiye kümesi payı | [%xx] | ≤ %40 | PASS/FAIL |
| Tek-morfem-ailesi payı (-anza vb.) | [%xx] | ≤ %40 | PASS/FAIL |
| Küme-içi karışabilirlik (pair payı) | [%xx] | ≤ %20 | PASS/FAIL |
| Farklı sonlanış-sınıfı sayısı | [n] | ≥ 3 | PASS/FAIL |

**Kapı sonucu:** `PASS` (finalist analizine geç) / **`FAIL` → ikinci tur tetiklendi** (eksik territory'ler: [liste]).

---

## 3. Dilbilimsel Laboratuvar — Finalist Analizi

Aşağıdaki **3–5 finalist**, yukarıdaki havuzdan SMILE+M / SCRATCH filtresi sonrası seçilmiştir.

### Finalist 1: **[İSİM]**

| Kriter | Değer |
|---|---|
| **Hece yapısı** | [örn: "Nai-ke", 2 hece, trochaic] |
| **Fonetik profil** | [örn: açık vokal sonu / Bouba-Kiki: spiky] |
| **[v2.1] İki-eksen skor** | Telaffuz-kolaylığı [xx/100] · Marka-gücü [yy/100] · Kompozit [zz/100] — *(eksenler AYRI; kolay-telaffuz ≠ güçlü-marka)* |
| **SMILE+M skoru** | S [✓] M [✓] I [✓] L [✓] E [✓] +M [✓] = 6/6 |
| **SCRATCH bayrakları** | [hiçbiri / X kategorisinde uyarı + gerekçe] |
| **3-katmanlı dilution** | L-A Levenshtein [temiz] / L-B Phonetic [temiz] / L-C Aura [temiz] |
| **[v2.1] Naif ilk-okuma (algı)** | Niyet-kök: [örn. ortus] → Algı-kök: [örn. **"orta" = vasat, OLUMSUZ**] — *sapma/olumsuz algı FIRST-CLASS* |
| **Telaffuz testi (6 dil)** | EN [✓] ES [✓] DE [✓] FR [✓] TR [✓] JP [✓] |
| **Disaster check (9 dil)** | EN/ES/AR/RU/ZH/TR/FR/DE/PT — temiz |
| **EN rationale** | *"[tek cümle uluslararası investor pitch tonu]"* |
| **TR rationale** | *"[tek cümle yönetim kurulu sunum tonu]"* |
| **Genişleme alanı (Legs)** | [3 olası alt-marka örneği] |

### Finalist 2: **[İSİM]**
[Aynı tablo]

### Finalist 3: **[İSİM]**
[Aynı tablo]

[... 4. ve 5. finalistler eğer varsa]

---

## 4. Domain ve Trademark Stratejisi

### 4.1 Domain Önerileri (öncelik sırası) — [v2.1] doğrulama-durumu ZORUNLU

**Bağlayıcı kural (v2.1):** Her `.com` hücresi **doğrulama-durumu + kaynak + zaman damgası** taşır. **Hiçbir alan adı iki uyuşan canlı kaynak (GoDaddy MCP, ya da RDAP + WHOIS) olmadan "müsait" YAZILMAZ.** Tek kaynak = `provisional`; kaynak yok = `unverified` (asla "müsait").

| Finalist | .com | Doğrulama-durumu | Kaynak(lar) | Kontrol zamanı (UTC) | Alt. TLD |
|---|---|---|---|---|---|
| [İsim 1] | ✓ MÜSAİT / ✗ DOLU / BELİRSİZ | confirmed \| provisional \| unverified | GoDaddy MCP · RDAP · WHOIS | [ISO-8601] | .ai/.io durumu |
| [İsim 2] | ... | ... | ... | ... | ... |

> **Durum etiketleri (asla varsayımsal değil):**
> - **confirmed_available** → yalnızca **≥2 canlı kaynak** boş teyit ettiyse "MÜSAİT" yaz.
> - **confirmed_taken** → herhangi bir canlı kaynak kayıt döndürdüyse "DOLU".
> - **provisional** → tek kaynak boş gösterdi → *"tek kaynak; ikinci doğrulama gerekli"* — **"müsait" DEME**.
> - **unverified** → canlı kaynak erişilemedi (offline) → *"doğrulanamadı"* — **asla "müsait" varsayma**.
>
> **GoDaddy MCP etkinse** birincil canlı kaynaktır; değilse `scripts/domain_recon.py` **RDAP (birincil) + WHOIS (ikincil)** ile iki-kaynak doğrulaması yapar. İkisi de yoksa manuel URL'ler:
> - [Namecheap WHOIS](https://www.namecheap.com/domains/whois/) · [Domainr](https://domainr.com) · [Instant Domain Search](https://instantdomainsearch.com) · [RDAP](https://rdap.org/)
>
> ⚠️ **Denetim dersi:** `auronza.com` ve `nortanza.com` DOLU'dur; tek-sinyal heuristic "müsait" der — **bu yasak**. Heuristic yalnız "rekabet seviyesi" tahminidir, müsaitlik hükmü değildir.

### 4.2 Trademark Pre-Screening

| Finalist | WIPO Madrid Sınıf | USPTO TESS | EUIPO eSearch+ | TÜRKPATENT |
|---|---|---|---|---|
| [İsim 1] | Class 9 (software) + Class 42 (SaaS) | [URL] | [URL] | [URL] |
| [İsim 2] | ... | ... | ... | ... |

> **Yasal uyarı**: Bu rapor **trademark hukuki tavsiyesi değildir**. Final kayıt öncesi trademark avukatı doğrulaması zorunludur.

### 4A. [v2.0] Pharma-Specific Regulatory Screening (pharma-only)

**Yalnızca pharma/biotech/medtech brief'lerinde yazılır.**

| Finalist | USAN Stem Çakışması | **[v2.1] Marka Çakışması (INN ötesi)** | LASA Riski (Top 3 Benzer Onaylı İlaç) | Promosyonel Yüklü Kelime | TİTCK TR Riski |
|---|---|---|---|---|---|
| [İsim] | ✗ / ✓ (stem adı) | ✗ / ✓ (marka + INN + yargı bölgesi) | [ilaç1 (Levenshtein), ilaç2, ilaç3] | ✗ / ✓ | ✗ / ✓ |

> **[v2.1] Marka çakışma taraması (uzman-denetim düzeltmesi E):** INN stem taraması (`inn_stem_collision.py`) **yeterli değildir** — denetimde `Claranta` stem taramasını geçti ama **klaritromisin/Claritin marka ailesiyle** çakışır ve Hindistan'da **tescilli Sınıf 5** markadır. Bu yüzden `scripts/pharma_brand_collision.py` mevcut **ilaç MARKA adlarına** karşı ayrı bir tarama yapar. Her satır **provenance** (taranan kaynaklar + `live_tm_checked:false`) taşır.
>
> **"Çakışma yok" ASLA tek yüzeysel aramadan verilmez.** Skorun 2/2 olması bile *"resmî çok-yargı-bölgeli TM araştırması gerekli — ön-tarama yeterli değil"* notuyla yazılır; **"temiz" kesin hükmü YASAK**.
>
> **Pharma yasal uyarı**: Pharma brand name **FDA Brand Name Review** / **EMA Invented Name Group** / **TİTCK** onayından geçmeden kullanılamaz. Bu skill ön-tarama yapar; formal regulatory submission zorunludur.

---

## 5. [v2.0] Post-Digital Readiness Scorecard

Her finalist altı eksenli skorlamaya tabidir. Her eksen 0–2 puan; toplam 0–12 (pharma-only eksen N/A ise 0–10).

### 5.1 Özet Tablo

| Finalist | Eksen 1 LLM | Eksen 2 Morpheme | Eksen 3 Voice | Eksen 4 Entity | Eksen 5 INN | Eksen 6 Dilution | **Toplam** |
|---|---|---|---|---|---|---|---|
| [İsim 1] | 2 | 2 | 2 | 2 | 2/N.A. | 2 | **12 / Digital-Safe** |
| [İsim 2] | 2 | 1 ⚠️ | 2 | 2 | N/A | 2 | **9 / Acceptable-with-note** |
| [İsim 3] | 1 | 2 | 1 ⚠️ | 2 | 2 | 2 | **10 / Digital-Safe** |

**Yorum eşikleri**:
- **10–12**: Digital-Safe — birinci tercih adaylığı güçlü
- **7–9**: Acceptable with notes — trade-off raporlanır
- **<7**: Demote — alternatif finalist aranır

### 5.2 Per-Finalist Detaylı Rationale

**Finalist 1: [İSİM] — Toplam 12/12**
- **Eksen 1 LLM Namespace (2/2)**: [ChatGPT/Claude/Gemini/Perplexity proxy sonuçları — temiz namespace]
- **Eksen 2 Morpheme Saturation (2/2)**: [korpus kalıp frekansı ve rationale]
- **Eksen 3 Voice/ASR (2/2)**: [ASR fidelity, homofonik çakışma yokluğu]
- **Eksen 4 Entity Disambiguation (2/2)**: [Wikipedia/Wikidata temizliği]
- **Eksen 5 INN/USAN (2/2 veya N/A)**: [INN stem tarama sonucu]
- **Eksen 6 Famous-Mark Dilution (2/2)**: [Layer A/B/C temizliği]

**Finalist 2: [İSİM] — Toplam 9/10 (pharma N/A)**
- **Eksen 1 (2/2)**: ...
- **Eksen 2 (1/2) ⚠️**: [suffix "-X" korpusta N kez — saturation uyarısı]
- ...

---

## 6. Stratejik Tavsiye Matrisi (v2.0 — 2 yeni sütun)

| Finalist | Kategori | SMILE+M | SCRATCH | Domain | Trademark | **Post-Digital** | **Pharma-Reg** | **Genel Öncelik** |
|---|---|---|---|---|---|---|---|---|
| [İsim 1] | C - Coined | 6/6 | Temiz | .com ✓ Yüksek | Yüksek | 12/12 | OK | **★★★★★ Birinci tercih** |
| [İsim 2] | E - Contrarian | 5/6 | Temiz | .com ✓ Orta | Orta | 9/10 | OK | **★★★★☆ Güçlü ikinci** |
| [İsim 3] | A - Metaforik | 5/6 | Soft-S | .com ✓ Yüksek | Yüksek | 10/12 | OK | **★★★★☆ Yedek tercih** |
| [İsim 4] | B - Portmanteau | 6/6 | Temiz | .com ✗ Orta | Orta | 7/12 ⚠️ | Soft-query | **★★★☆☆ Alternatif** |

---

## 7. Sonraki Adımlar + Yöntemsel Notlar

### 7.1 Sonraki Adımlar (Recommended Process)

1. **Müşteri shortlist seçimi** (3 → 1): Yönetim kurulu / kurucu inceleme oturumu (1-2 hafta)
2. **Yerel pazar testi**: Hedef coğrafyalardaki 3-5 native speaker "voice of the stakeholders" exercise
3. **Comprehensive trademark search**: Trademark avukatı + Corsearch / Compumark
4. **Domain alımı**: Final isim onaylandıktan **24 saat içinde** domain rezervasyonu
5. **WIPO Madrid başvurusu**: Coğrafi öncelik tarihi için TÜRKPATENT veya USPTO ilk başvuru, ardından Madrid extension
6. **Linguistik audit**: Mandarin, Arabic, Japanese gibi non-Latin pazarlarda yerel dilbilimci
7. **[v2.0] Pharma-spesifik**: FDA DMEPA brand name review başvurusu (8–12 ay timeline)
8. **[v2.0] Brand-visual handover**: Final isim seçildikten sonra `brand-visual` skill'i ile visual identity system

### 7.2 Yöntemsel Notlar

Bu rapor aşağıdaki kanonik kaynaklara dayalıdır:

- Ries, A. & Trout, J. (1981). *Positioning*. McGraw-Hill.
- Wheeler, A. (2013). *Designing Brand Identity* (4th ed.). Wiley.
- Watkins, A. (2014). *Hello, My Name is Awesome*. Berrett-Koehler.
- Miller, D. (2017). *Building a StoryBrand*. HarperCollins.
- Burmann, C. et al. (2017). *Identity-Based Brand Management*. Springer Gabler.
- Yuen, R. (2024). *Decoding Branding* (2nd ed.). Routledge.
- Meyerson, R. (2024). *Brand Naming*. Heirloom Agency Publishing.
- Neumeier, M. (2024). *Scratch: The Contrarian Branding Book* (rev. ed.). New Riders.
- Interbrand. (2024-2026). *Best Global Brands Reports*.
- Pentagram. (2025). *AI + Brand Craft Whitepaper*.
- Wolff Olins. (2025). *Fluid Identity*.

Skill versiyonu: brand-maker v2.0
```

---

## Format Disiplini Notları

### Hangi Bölümler Bağlayıcı, Hangileri Esnek?

| Bölüm | Durum |
|---|---|
| 1. Stratejik Deşifre | **Bağlayıcı** — atlanamaz |
| 2. Beş-Kategori Bazlı Aday Havuzu | **Bağlayıcı** — minimum 2 kategoride aday |
| 2.1 Çeşitlilik Kapısı (v2.1) | **Bağlayıcı v2.1** — finalist analizinden önce; FAIL → ikinci tur |
| 3. Dilbilimsel Laboratuvar | **Bağlayıcı** — her finalist için tablo zorunlu (naif ilk-okuma satırı dahil) |
| 4. Domain ve Trademark Stratejisi | **Bağlayıcı** — doğrulama-durumu + kaynak + zaman damgası (v2.1); manual URL'ler şart |
| 4A. Pharma-Specific | **Bağlayıcı pharma-only** — INN stem + marka çakışması (v2.1) + provenance |
| 5. Post-Digital Readiness Scorecard | **Bağlayıcı v2.0** — her finalist için 6-eksen |
| 6. Stratejik Tavsiye Matrisi | **Bağlayıcı** — özet tablo |
| 7. Sonraki Adımlar | **Önerilir** |
| 7.2 Yöntemsel Notlar | **Bağlayıcı** — kaynak göstermek bilimsel disiplin |

### Uzunluk Hedefi (v2.0 Güncellendi)

- Küçük brief (single product, niş pazar): **2.000–2.500 kelime**
- Standart kurumsal brief: **2.500–3.500 kelime**
- Born-global startup launch: **3.500–4.500 kelime**
- Pharma brand launch: **3.500–5.000 kelime** (Section 4A ve Section 5 detay)

### Ton Kontrolleri

- ❌ "Belki", "sanırım", "düşünüyorum" → çıkar
- ❌ "Çok güzel", "harika bir isim" → ölçülebilir terimlerle değiştir
- ✅ "Bu isim, [SMILE+M 6/6 + SCRATCH temiz + Post-Digital 11/12] sağladığı için [stratejik gerekçe] olarak öneriyorum"
- ✅ Sayısal eviden (skor, hece sayısı, dil sayısı) her zaman önce

### Markdown Disiplini

- Başlık seviyeleri: H1 (rapor başlığı), H2 (ana bölümler 1-7), H3 (alt bölümler), H4 (finalist isimleri)
- Tablolar: standart markdown table syntax
- Kalın: finalist isimleri ve stratejik kelime sahiplenmeleri
- İtalik: rationale cümleleri
- Yatay çizgi (`---`): bölümler arası ayraç
