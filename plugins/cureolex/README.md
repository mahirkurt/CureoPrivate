...
>
> **Sürüm 3.8.7 (2026-08-18):** Cursor native MCP Bearer `${env:VAR}` process-env interpolasyonu (`.cursor-plugin/mcp.json`) — IDE plugin-variable yapıştırma formu açmaz. Claude Code `.mcp.json` `${VAR}` kalır. Önceki: **3.8.6** ikinci semantik geçiş (uk-legal `legislation_*`, UHRI, fedlex Vernehmlassung, mevzuat 0.15.1).
>
