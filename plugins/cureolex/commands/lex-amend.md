...
`cureolex` flagship skill'ini **Mod 2 AMEND** olarak çalıştır.
...
4. **AMEND 8-adım:** hedefi çek → `get_onceki_metinler` → değişiklik tipini sınıfla (Md.19 madde / Md.20 ibare / Md.16 ek-geçici / Md.21-8 ilga) → çerçeve-madde (Md.18 eski→yeni) → atıf (Md.21 sıfırsız tarih) → **üst-norm/Md.90/5:** `treaty_status`/`treaty_reservations` + `coe_treaty_signatories`(164/211) + `intl_treaty_info` (onay uydurulmaz) → karşılaştırma cetveli → madde gerekçesi → R9 QC.
5. **Template:** [`templates/karsilastirma-cetveli.md`](../skills/cureolex/templates/karsilastirma-cetveli.md) + [`templates/madde-gerekce.md`](../skills/cureolex/templates/madde-gerekce.md).
