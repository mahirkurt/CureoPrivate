...
Cureolex'ın **tam-filo ilkesi** (wire'lı tüm araçlar her sorguda çalışır) için hangi katmanın gerçekten hazır olduğunu **canlı prob'la** raporla — env-var varlığına bakarak DEĞİL.
...
> anamnesis bir *kaynak* değil, bağlam-ekonomisi Tier 2 RAG substratıdır
> (`collection=cureolex:sess:<id>` + önekli doc_id; `doc_scope` yoktur).
> Anahtarı yoksa büyük belge işleme bounded-chunk fallback'e degrade eder
> (`references/16` §C) — plugin yine çalışır.
