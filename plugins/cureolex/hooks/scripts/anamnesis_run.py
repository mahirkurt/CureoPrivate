...
def context_message(ledger: dict) -> str:
    ...
    return (
        ...
        f"doc_id='{example}'). "
        "cureolex işini yaparken hybrid_query / semantic_search collection "
        "VE/VEYA önekli doc_ids[] ile çağrılır; kapsamsız çağrı YASAK "
        "(paylaşılan indeks sızıntısı) — bu kural filo geneli ve mutlaktır. "
        "graph_* aynı kural. "
"Aynı oturumda başka bir plugin kendi koleksiyonunu bildirirse ÇELİŞKİ YOKTUR: "
        "her mandat yalnız kendi plugin'inin işi için bağlayıcıdır ve guard'lar "
        "birbirini DENY etmez — o plugin'in işini yaparken onun koleksiyonu geçerlidir. "
