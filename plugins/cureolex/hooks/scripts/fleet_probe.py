...
def roster_fingerprint(lock) -> str:
    ...
    name+url+auth_env değişince damga değişir → cache ölür. `lock` yok/bozuksa
    boş damga (fail-open; okuyucu yine damga isteyince legacy dosyayı reddeder).
    """
    rows = []
    for s in (lock or {}).get("servers") or []:
        rows.append((str(s.get("name") or ""),
                     str(s.get("url") or ""),
                     str(s.get("auth_env") or "")))
    rows.sort()
    blob = json.dumps(rows, separators=(",", ":"), ensure_ascii=False)
    return hashlib.sha256(blob.encode("utf-8")).hexdigest()


def read_cache(path, ttl: int = CACHE_TTL, roster=None):
    """Taze cache'i döner; bayat/bozuk/eksikse veya roster damgası uyuşmazsa None.

    `roster` verilirse dosyadaki damga birebir eşleşmeli — damgasız (eski biçim)
    dosya güvenilmezdir, yeniden prob edilir. `roster=None` eski çağrıları
    (yalnız yaş) bozmaz.
    """
    try:
        ...
        if time.time() - float(data["ts"]) > ttl:
            return None
        if roster is not None and data.get("roster") != roster:
            return None
        return data["results"]
    except Exception:
        return None
...
def write_cache(path, results: dict, roster=None) -> None:
    try:
        path = Path(path)
...
def cached_probe(root, env, ttl: int = CACHE_TTL, fresh: bool = False) -> dict:
    ...
    try:
        ...
        stamp = roster_fingerprint(lock)
        if not fresh:
            cached = read_cache(path, ttl, stamp)
            if cached is not None:
                return cached
        results = probe_fleet(lock, env)
        write_cache(path, results, stamp)
        return results
    except Exception:
        return {}
