#!/usr/bin/env python3
"""cureolex PostToolUse advisory — retrieve-don't-dump + evidence_index (hukuk metni).
...
Tier 0 ihlali). İki katmanlı yönlendirme (shared/context-economy-contract.md):
  ...
  * BÜYÜK-eşik üstü (>big_threshold): tek belge çok büyük →
    `anamnesis.ingest_document(collection=cureolex:sess:<id>, doc_id=<collection>:<kanonik>)`
    ile Tier 2 RAG substratına indeksle → hybrid_query(collection=…, doc_ids=[…], queries[])
    ile bounded, provenance-damgalı dilim çek (evidence_index). doc_scope YOKTUR.
    Aynı önekli doc_id iki kez ingest edilmez (kanonik cache).
...
def main():
    ...
    if n > big:
        ...
        coll, example = "cureolex:sess:<session>", "cureolex:sess:<session>:mevzuat:<kanonik>"
        try:
            sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            from anamnesis_run import load_ledger, scoped_doc_id
            led = load_ledger()
            if led:
                coll = led["collection"]
                example = scoped_doc_id(coll, "mevzuat:<kanonik>")
        except Exception:
            pass
        msg = (
            "[cureolex] büyük-veri: '{base}' ~{kb} KB tek büyük belge döndürdü (Tier 0 sınırı aşıldı). "
            "Bunu HAM işleme. Tier 2 RAG substratına indeksle: "
            "anamnesis.ingest_document(collection='{coll}', "
            "doc_id='{example}', text=<gövde>) → "
            "anamnesis.hybrid_query(collection='{coll}', doc_ids=['{example}'], "
            "queries=[<hedef maddeler/hükümler>]) ile bounded, provenance-damgalı dilim çek "
            "(doc_scope YOKTUR; cevaplar doc_id::idx). Aynı önekli doc_id iki kez ingest edilmez "
            "(kanonik cache; shared/context-economy-contract.md §3). anamnesis anahtarı yoksa → "
            "bounded-chunk fallback: get_mevzuat_madde_tree ile hedefi lokalize et, sonra yalnız "
            "o maddeyi/sayfayı çek (references/16 §C). İçtihat = reform-GEREKÇE sinyali, dava dilekçesi değil."
        ).format(base=base, kb=round(n / 1024, 1), coll=coll, example=example)
    else:
        # Tier 1 — distiller alt-ajanına delege.
        msg = (
            "[cureolex] retrieve-don't-dump: '{base}' ~{kb} KB büyük hukuk-metni döndürdü. "
