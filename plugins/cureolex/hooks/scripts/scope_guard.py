...
cureolex YALNIZ mevzuat reformu içindir (SKILL §6). Bireysel hak-arama (SGK ödeme reddi davası,
...
def main():
    ...
    if not prompt or not CONTEXT.search(prompt):
        sys.exit(0)  # cureolex bağlamı yok → sessiz
    ...
    ctx = (
        "[cureolex Scope Guard] Bu talep KAPSAM-DIŞI bir sinyal taşıyor olabilir → "
        + "; ".join(hits)
        + ". cureolex yalnız mevzuat REFORMU/üretimi içindir (DRAFT/AMEND/analiz/uyum/görüş/"
        "RIA/karşılaştırmalı/TBMM/ex-post). Talep gerçekten bireysel hak-arama veya promosyon "
        "denetimi ise yukarıdaki skill'e yönlendir, cureolex çıktısı ÜRETME. Reform bağlamıysa "
