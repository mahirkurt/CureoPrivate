...
description: >-
  ...
  "karşılaştırmalı analiz / AB karşılığı", "ex post değerlendirme". 22 wire'lı MCP + 3 companion (Yargı ·
...
3. **TAM-FİLO devreye al (§3 — zorunlu, her sorgu).** Wire edilmiş **22 kaynak MCP + 3 companion**'un tamamı her sorguda taranır. Getirimi **≤4 paralel shard**'a böl (TR-çekirdek · karşılaştırmalı · doktrin · klinik), her shard'ı bir distiller alt-ajanına ver; ana bağlama yalnız kompakt `retrieval_distillate` + `coverage` döner. Bağlam ekonomisi ve büyük-veri disiplini **zorunludur** (§3.5) — ham veri ana pencereye girmez. Hiçbir server sessizce atlanmaz.
4. **Üst-norm zincirini kur.** `mcp__mevzuat__get_anayasa` → dayanak kanun/CBK → yönetmelik. Primer arama: anahtar kelime **ve** kanun/mevzuat NUMARASI (`search_mevzuat` `mevzuat_no` veya yalnız-rakam query), `phrase` (bedesten Solr), belge-içi `search_within_mevzuat`. Ayrıntı: `references/07-mevzuat-mcp-workflow.md`. **Kesim:** 0.15.0 yerel pakettedir; HP production henüz kesilmedi — canlı primer 0.14.x ise phrase / `search_within` / tam gerekçe o uçta yoktur (uydurma; ikincil veya `manual_required`).
...
Wire edilmiş 22 server **`fleet.yaml`'de tanımlıdır ve `.mcp.json` ondan üretilir** (19'u Bearer-anahtar-gated; `mevzuat-bilgisi`, `yoktez`, `literatur` public). Filo tanımının tek kaynağı budur — hook anahtar haritası, `/lex-connectors` tablosu, distiller araç kısıtları ve mod×server matrisi hep ondan türer; `tools/fleetkit/check_drift.py` (repo kökü) türetilmiş≠commit'li hâlini yakalar — bu araç **yalnız kaynak depoda vardır, kurulu plugin'de yoktur**; kurulu pakette `fleet.yaml` ve türevleri salt-okunur kanıttır. **Temel kural: hepsi her sorguda devreye alınır.** Primer/ikincil ayrımı artık *aktivasyon kapısı* değil, **sentezde otorite önceliğidir** — çatışmada primer kazanır, ama ikincil/support da **çalıştırılır** ve sonucu kapsam manifestosuna girer. Bir server'ın atlanması yalnız iki halde meşrudur: (a) o server için anahtar yok (SessionStart preflight işaretledi), (b) mod için mantıksal olarak N/A — **her iki hal de manifestoda gerekçesiyle beyan edilir** (sessiz atlama = G0 FAIL).

- **TR primer mevzuat:** `mcp__mevzuat__*` — yapısal yasama-grafı (madde_tree, madde_diff as_of, timeline, ilga_zinciri, relations) **ve** first-party bedesten: kanun/mevzuat NUMARASI lookup, `phrase` (Solr), `search_within_mevzuat` (belge-içi AND/OR/NOT; EK/GEÇİCİ ağacı), `get_mevzuat_gerekce` (TBMM locator **+** bedesten tam gerekçe metni), `get_mevzuat_content`. Playwright clone yok (CB karar/genelge PDF markdown, OpenRouter semantik `search_within` primerde **yoktur**). **İkincil (wire'lıysa her sorguda çapraz-kontrol):** `mcp__mevzuat-bilgisi__*` (surucu.dev) — bağımsız ikinci oy; çatışmada primer kazanır.
  - **Kesim notu (zorunlu, canlı uç):** mevzuat-mcp **0.15.0 yerel pakettedir**; HP production primer henüz 0.15.0'a kesilmedi. Bu oturum `mevzuat.cureonics.com` (canlı 0.14.x) ise phrase / `search_within_mevzuat` / tam gerekçe o uçta **yoktur** — tool listesinde yoksa varmış gibi yazma; ikincil (wire'lıysa) veya `manual_required`.
  - **DEVRALMA KURALI (v3.5.4 — zorunlu; primer yetenekleri 0.15.0).** `mcp__mevzuat__*` **düşükse** (upstream TLS/erişim arızası, origin timeout, boş `items` + dolu `diagnostics`), Türk mevzuatı yükünü `mcp__mevzuat-bilgisi__*` **devralır** — çıktı durmaz. Üç koşul birlikte zorunludur: (a) manifestoda `mevzuat → degraded: <teşhis>` **ve** `mevzuat-bilgisi → hit N (fallback devraldı)`; (b) yedekten gelen her atıf `source: mevzuat_bilgisi_mcp (fallback)` etiketi + birincil-doğrulama açığı notu taşır; (c) birincil ayağa kalkınca **çapraz-doğrulanır**. **Yedek EŞDEĞER DEĞİLDİR:** yapısal graf (`madde_tree`/`madde_diff`/`timeline`/`relations`/`ilga`) ve **`resolve_resmi_gazete` yalnız primerdedir** — RG çapa yedekle KAPANMAZ (kapatılamıyorsa `manual_required`). **Yedek TBMM yasama tarihçesini (teklif/önerge/tutanak) KAPATMAZ** — o yalnız `mcp__tbmm__*`'tedir; SPA/`related_acik_erisim` canlı Genel Kurul SAYILMAZ. İkincil `search_mevzuat` AND/OR/NOT desteklemez (`+terim1 +terim2`); belge-içi AND/OR/NOT primere `search_within_mevzuat` ile aittir (canlı 0.14.x uçta yoksa ikincilin kendi `search_within`'i çapraz olabilir — çatışmada primer). İkincilin Playwright-yalnız yüzeylerini primere mal etme. **İkisi de düşükse Türk mevzuatı iddiası ÜRETME.** Kayıt: `source_registry.yaml → mcp.mevzuat_bilgisi`.
...
- **Karşılaştırmalı:** `mcp__health-policy__*` (yabancı ülke resmî-metin — US/CA/JP/AU/ES/IE/CN/MX), `mcp__german-law__*` (DE + AB↔DE uygulama haritası; günlük kısaltmalar `AMG`/`BDSG` çözülür, çözülemezse `{found:false}` — sessiz null yok), `mcp__eurlex__*` (**G6 CELEX taşıyıcısı** — `eurlex_lookup_celex`), `mcp__uk-legal__*` (UK içtihat/Hansard; mevzuat metni `legislation_*` değil `ep.legislation_uk`), `mcp__fedlex__*` (CH birincil metin, SR-numaralı), `mcp__ich-guidelines__*` (**M4 CTD / M8 eCTD spesifikasyonu IN** — COMPARATIVE ve DRAFT dosya-yapısı hukuki/regülatuar atıfı; şirket sequence / "0001 gönderildi" iddiası yok), `mcp__intl-treaty__*` (Md.90/5 — ateş listesi §3 Anayasa Md. 90/5), `mcp__eudamed__*` (AB cihaz), `mcp__oecd__*` (RIA nicel).
...
- UHRI `tools_used` **dışıdır** (sorgu API yok; 109 MB dump çekilmez). UN tavsiyesi gerekirse coverage'a UHRI portal deep-link.
...
- **Companion (dış connector — tam-filonun ZORUNLU üyeleri; bkz. `/lex-connectors`):** yalnız **üç** tanedir: Yargı içtihat (`mcp__Yarg__*`), Open Law (artık yalnız UK birincil metin + HUDOC, `mcp__Open_Law__*`), Ansvar (58-yargı tarama, `mcp__Ansvar__*`). Kararlı self-host URL'leri yoktur; claude.ai/Cursor connector ayarlarından bağlanır. **Fedlex 2026-08-08'de wire edildi** (`mcp__fedlex__*`) — companion DEĞİLDİR. Türk Patent wire'ı 2026-08-17'de emekli edildi (ölü Capsolver upstream). *(Önek notu: claude.ai connector öneki yüzeye göre `mcp__Yarg__*` ya da `mcp__claude_ai_Yarg__*` görünebilir — eşleştirmeyi server ADINA göre yap.)* Bağlı companion "opsiyonel" DEĞİLDİR — tetiklenmiş bağlamda çağrılmaları zorunludur:
  ...
  - **Open Law ↔ UK (G6 DEĞİL):** UK birincil metin + HUDOC çapraz kontrolü. **G6 CELEX artık wire'lı `mcp__eurlex__eurlex_lookup_celex`'tedir** (hard PASS, yoktez geçişiyle aynı mantık). Open Law bağlı değilse UK metni `ep.legislation_uk`'ye degrade eder; G6'yı düşürmez. german-law `get_eu_basis` CELEX üretmez — G6 yedeği değildir.
...
|---|---|---|
| **G0** | Tam-filo kapsam (§3) | Wire edilmiş 22 MCP + **3 companion (Yarg/Open_Law/Ansvar — zorunlu satırlar)** + evidentia (klinik-boyut varsa) + sci-audit **tamamı** ateşlenmiş; her biri kapsam manifestosunda hit/empty/degraded/skipped-with-reason olarak görünür. Companion/plugin `skipped` yalnız gerçek yoklukta meşrudur ve ilgili kapıyı (G5/G6) CONDITIONAL'a düşürür. intl-treaty `live_coe:false` **beyanlı degrade** (`degraded: snapshot`) — sessiz atlama = FAIL. |
