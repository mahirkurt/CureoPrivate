...
- **TR primer mevzuat:** `mcp__mevzuat__*` — semantik sıra: `list_mevzuat_types`→`list_mevzuat_by_type`/`search_*`→`get_mevzuat_detail`→yapısal graf→`search_within`→content→`get_mevzuat_gerekce`→`resolve_resmi_gazete`→`list_kurumlar`. Yapısal graf + bedesten (NUMARA, `phrase`, `search_within`, gerekçe). **İkincil:** `mcp__mevzuat-bilgisi__*` çapraz.
