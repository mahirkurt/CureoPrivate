...
**Anayasa Md. 90/5 + Uluslararası İnsan Hakları Sözleşmeleri (R10) — MCP ateş:**
- [ ] **`treaty_status` + `treaty_reservations` çağrıldı mı?** (ICESCR IV-3, ICCPR, CEDAW, CRC, CRPD + dosyanın adlandırdığı chapter-IV alias; onay/çekince uydurulmadı)
- [ ] **`coe_treaty_signatories` çağrıldı mı?** (Oviedo CETS 164 + MEDICRIME CETS 211; snapshot + `snapshot_age_days` + `mcp_verified:false`; `live_coe:false` beyanlı degrade)
- [ ] **`intl_treaty_info` dosya başına bir kez çağrıldı mı?** (coverage manifesto)
...
- [ ] **Klinik araştırma / sahte ilaç boyutu varsa Oviedo 164 / MEDICRIME 211 taraf statüsü snapshot'tan okundu mu?** (stale "taraf değil" tablosu kullanılmaz)
...
- Kontrol **21 (G-DİL)** FAIL ise → metin **Cureolex kalite eşiğinin altında**; yeniden yazım gerekir. Özellikle Anayasa Md. 90/5 uyumu (ICESCR/CRC/CRPD) FAIL ise **AYM bireysel başvuru zafiyeti**; AİHM ihlal riski.
