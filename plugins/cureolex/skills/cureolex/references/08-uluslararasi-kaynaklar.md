...
Bu dosya, Cureolex protokolünün **Bölüm 5.4 (Uluslararası Kaynak Katmanı)** reçetesinin operasyonel sözlüğüdür. Her kaynak için **(a) kurum/ veri tabanı tanımı, (b) URL örüntüsü, (c) erişim modalitesi, (d) Cureolex modu eşlemesi, (e) tipik sorgu reçetesi** verilmiştir.
...
## 0. Cross-Reference Index — Cureolex v2.0
...
**Cureolex modu eşlemesi:** DRAFT (Adım 5), ANALYZE (Adım 4), OPINE (Adım 3 — AB Başkanlığı), RIA (Adım 4).
...
**Cureolex modu eşlemesi:** DRAFT (bilimsel temellendirme), RIA (klinik benchmark), OPINE (TİTCK görüşünde EMA paralelinin gösterilmesi).
...
**Cureolex modu eşlemesi:** DRAFT (cihaz yönetmeliği taslakları için), RIA (ÜTS karşılaştırması).
...
**Cureolex modu eşlemesi:** ANALYZE (ruhsat prosedürü uyumu), OPINE (TİTCK ruhsat süreçleri ile karşılaştırma).
...
**Cureolex modu eşlemesi:** RIA (SUT geri ödeme tebliği taslaklarında JCA çıktısının doğrudan kullanımı), OPINE (SGK görüşünde JCA referansı).
...
**Cureolex modu eşlemesi:** DRAFT, ANALYZE, RIA (AB üye devleti karşılaştırması için varsayılan ilk seçim).
...
**Cureolex modu eşlemesi:** DRAFT, RIA (AB üye devleti karşılaştırması ikincil varsayılan).
...
**Cureolex modu eşlemesi:** Tüm modlar; Brexit sonrası UK MDR Türkiye için kritik referans (AB-dışı ama gelişmiş, AB ile yapısal benzerlik).
...
**Cureolex modu eşlemesi:** RIA üçüncü AB üye devleti karşılaştırması.
...
**Cureolex modu eşlemesi:** Güney Avrupa karşılaştırması; pediatrik düzenleme ve generik politika için referans.
...
**Cureolex modu eşlemesi:** **Yapısal benchmark olarak özellikle önemli** — Türkiye'nin AB-dışı ama AB-uyumlu pozisyonuyla en yakın paraleli İsviçre'dir.
...
**Cureolex modu eşlemesi:** DRAFT, ANALYZE, RIA.
...
**Cureolex modu eşlemesi:** RIA (ABD'de etki analizi metodu — NPRM yorum dönemi + Final Rule).
...
**Cureolex modu eşlemesi:** RIA (etki analizi metriği — FAERS sinyali → mevzuat revizyon ihtiyacı), DRAFT (farmakovijilans yönetmelik benchmark).
...
**Cureolex modu eşlemesi:** SUT taslakları için yapısal benchmark.
...
**Cureolex modu eşlemesi:** RIA bilimsel temellendirme katmanında.
...
**Cureolex modu eşlemesi:** RIA (SUT geri ödeme tebliği maliyet etkililik tartışması).
...
| Oviedo Sözleşmesi (CETS 164) | Tam metin | **Taraf** — `coe_treaty_signatories` snapshot (onay 02/07/2004); uydurma "imzalamadı" YASAK |
...
**Cureolex modu eşlemesi:** Pharmapatent skill ile birlikte; veri imtiyazı veya SPC yönetmelik taslaklarında.

### 5.4. World Bank — Open Data (data.worldbank.org)

**Tanım:** Sağlık harcaması, OOP, kapsam göstergeleri.

**Cureolex modu eşlemesi:** RIA sosyo-ekonomik etki bölümünde.
...
- ICH M4 — CTD formatı (modül yapısı, granularity, bölgesel M1 vs ICH M2–M5)
- ICH M8 — eCTD spesifikasyonu (sürümler, Q&A, değişiklik tarihi)
- ICH M11 — Klinik elektronik veri standartları

**Cureolex modu eşlemesi:** Türk Klinik Araştırma ve Ruhsatlandırma Yönetmeliklerinin bilimsel iskeleti büyük ölçüde ICH ile uyumludur — taslakta dolaylı/doğrudan referans gerekir. COMPARATIVE ve DRAFT, M4/M8'i **dosya-yapısı** hukuki/regülatuar atıfı için kullanır; "sequence 0001 gönderildi" iddiası üretmez (o ops `pharmaintel`).
...
**Cureolex modu eşlemesi:** ANALYZE (uyum analizi), OPINE (AB Başkanlığı görüşünde içtihat dayanağı), COMPARATIVE_LAW (yeni 7. mod) yargısal kanıt boyutunda.
...
**Cureolex modu eşlemesi:** ANALYZE iptal/ihlal risk değerlendirmesi, COMPARATIVE_LAW yargısal kanıt.
...
Almanca AMG ve Fransızca CSP'nin **İngilizce resmi çevirileri sınırlıdır**; otomatik çeviri sonuçları **terminolojik kesinlik açısından mevzuat amacıyla yetersizdir.** Cureolex çıktısında uluslararası referans verilirken **çift gösterim** kullanın:
