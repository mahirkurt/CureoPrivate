...
AB katmanı, birinci sınıf bir **Bağlı Açık Veri (Linked Open Data)** altyapısı sunar; Cureolex'ın AB erişimini kazımadan otomatik-doğrulanabilir sorgulara taşır.
...
MCP ekosistemi olgunlaşmıştır; aşağıdaki sunucular Cureolex'ın araç katmanını hızlandırabilir. **Benimseme kuralı:** Bir kaynak için API kararlıysa MCP sarmalayıcısı tercih edilir; web-only ise Fetch+parse fallback kullanılır.

| MCP sunucusu | Kapsam | Anahtar | Cureolex kullanımı |
|---|---|---|---|
...
| Standart | Tanım | Cureolex için anlam |
|---|---|---|
