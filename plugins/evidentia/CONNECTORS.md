...
| Connector | claude.ai | Auth | Anahtar araçlar | Güven | Katman |
|---|---|---|---|---|---|
| **openathens** (self-host) | 🟢 | OAuth/Bearer | `oa_resolve` · `oa_fetch_fulltext`(ingest) · **`oa_verify_access`** (DOI erişilebilir mi — getirmeden ÖNCE sor) · `oa_list_databases` (⚠️ canlıda `manual_required: databases_unreachable` — dürüst degrade) · `oa_session_status` · `oa_batch_submit`/`oa_batch_result` · `search`/`fetch` (deep-research sözleşmesi) — 10 araç, ölçüm 2026-08-07 — **tam-metin Tier 3: LİSANSLI kurumsal** (Millet Kütüphanesi/OpenAthens → ProQuest/EBSCO/ScienceDirect/Wiley/Nature/JSTOR/Cochrane…; **annas'ın ÖNÜNDE**, legal-öncelikli). **CANLI** `https://openathens.cureonics.com/mcp` (`openathens-mcp`, **anti-bot v2 2026-07-13**). Getirme **Xvfb headed** kalıcı-profil (gerçek parmak-izi + `cf_clearance`): anti-bot'suz (Springer/Nature) doğrudan; anti-bot'lu (Wiley/Elsevier/OUP/Sage/T&F) → non-interactive CF challenge kendiliğinden geçer (çoğu tam metin verir), yalnız interactive reCAPTCHA/Turnstile → `challenge_required` (operatör noVNC `oa-vnc.sh`; `manual_required`'dan ayrı) → çözülünce `cf_clearance` kalıcı. Bağlı değilse cascade Tier 4/5'e düşer | operatör self-host | O |
| annas-reader | 🟡 | — | article_search · read_article · book_search · get_document_info · search_in_document · read_document (⚠️ `*_download` YOK — efemeral okuyucu, 2026-08-07 ölçümü) (**tam-metin Tier 5 — SON ÇARE**, lisanslı band'dan [OpenAthens+Wiley] sonra; **copyright kapısı**) | topluluk | A |
...
| **Annas Reader** | `https://annas.cureonics.com/mcp` | **Tam-metin geri-çağırma** (operatör-bağlı Cloud Run, OAuth-gated; 2026-06-25 401 SECURED) — **§1.1 çekirdek tam-metin rung'unun parçası; full-text cascade Tier 5 (SON ÇARE)** — lisanslı band (OpenAthens Tier 3 + Wiley Tier 4) getiremeyince devreye girer (legal-öncelikli), generic `annas-reader` satırını gerçekler/yerine geçer. Araçlar: `article_search`/`article_download` (DOI), `book_search`/`book_download` (MD5+format). ⚠️ İndirmeler **kullanıcının makinesine** iner (sandbox'a değil) → analiz için **anamnesis ingest** veya yapıştırma gerekir. **Telif:** yalnız analiz, toplu birebir çoğaltma YOK. |
...
| **semantic-scholar** | `https://semanticscholar-mcp.cureonics.workers.dev/mcp` | ✅ SELF-HOST 2026-08-08 · 4 araç · 48 test · 4/4 canlı doğrulandı | **§6.3P akademik yüzeyi TAMAMEN kapandı** — pipeworx gateway'den operatör Worker'ına taşındı. Engel ölçümle beklenmişti (anahtarsız S2 `paper/search` residential IP'den 4/4 kez 429; gateway 200) → operatör `SEMANTIC_SCHOLAR_API_KEY` verince taşındı. **Yapısal kazanım:** gateway ~35 araç sunuyordu, whitelist 4'tü; Worker YALNIZ 4'ünü uyguluyor → en-az-yetki artık hook değil, sunucunun şekli. Anahtar tuttuğu için **KAPILI** (`SEMANTICSCHOLAR_MCP_API_KEY`). S2 anahtarlıyken bile ~1 req/s kısıtlar → sınırlı backoff; kalıcı 429 dürüstçe "THROTTLE, boş sonuç değil" der | operatör self-host | K |
...
| TR ruhsat/fiyat/biyobenzer | **TİTCK** (`titck.cureonics.com`, kapılı — bundle'daki tek TİTCK, doğrudan) → Mevzuat → bulunamazsa boşluk |
