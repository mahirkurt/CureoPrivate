...
| **anamnesis** | `https://anamnesis-mcp.cureonics.workers.dev/mcp` (✅ CANLI 2026-06-25 · RAG+GraphRAG E2E doğrulandı, corpus temiz) | **RAG/GraphRAG retrieval substratı** — registry'de hosted GraphRAG MCP yok (`rag-knowledge-graph-mcp` yalnız yerel stdio) → **self-host** (`self-host/anamnesis-mcp/BUILD-BRIEF.md`). Semantik chunking (bge-m3 1024-d) + Vectorize + D1 bilgi grafiği. **8 araç:** `ingest_document` (→ manifest, ham metin değil) · `semantic_search` · `upsert_triples` · `graph_neighbors` · `subgraph` · **`hybrid_query`** · `corpus_stats` · **`forget_document`** (doc_id ile temiz silme — Vectorize vektörleri + D1 chunks/manifest/edges + node-provenance küçültme/orphan-silme; idempotent, destructive; v1.4.1). **Bağlam-penceresi taşma koruması** (§3 → `evidence_index`). Deploy: Vectorize 1024-d cosine + D1 provizyon. |
