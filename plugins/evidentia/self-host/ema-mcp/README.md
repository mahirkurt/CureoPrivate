...
> pre-existing `ajv` CJS/ESM shim error in `@cloudflare/vitest-pool-workers` — the same failure
> affects the sibling `openfda-mcp`/`who-gho-mcp` and is environmental, not code. The unit suites
> pass. The live path is E2E-verified (Keytruda → Authorised/L01FF02/opinion 2015-05-20; orphan-L01
> filter; ema_stats 2712 medicines).
