 ...
 */
export class WhoGho extends DurableObject<Env> {}

/** Fresh server per request — no DO session SQL. */
function mcpServerFor(env: Env): McpServer {
  const server = new McpServer({ name: "who-gho-mcp", version: pkg.version });
  registerTools(server, env as unknown as WhoGhoEnv);
  return server;
}
