...
| Connector | Server | Verified primary tools | Notes |
|---|---|---|---|
| **openathens** (self-host) | `openathens.cureonics.com/mcp` (Bearer `${OPENATHENS_MCP_API_KEY}`) | `oa_resolve`, `oa_fetch_fulltext` (ingest), `oa_list_databases`, `oa_batch_submit`/`oa_batch_result` | Full-text cascade **Tier 3 — LICENSED institutional** (primary paywall gate, legal-first, ahead of annas); Millet Kütüphanesi/OpenAthens SAML; anti-bot publisher → `manual_required`; unbound → skip to Tier 4/5. Core P4 rung, not a domain module |
