# Execution Map — Ordered PRISMA Tool Playbook (v9.0.2)
...
**Fleet SSOT:** `../../fleet.yaml` → **19 servers** (10 gated / 9 public) + **9 companions**
...
| 2 | `pubmed-epmc` | P1.2 | MUST | MeSH + PubMed/EPMC search; P4 Tier 1/6 full-text |
...
| 5 | `openathens` | P4.T3 | MUST* | Licensed full-text (legal-first, before Wiley/annas) |
| 6 | `annas-reader` | P4.T5 | MUST* | Last-resort grey full-text after licensed band |
| 7 | `anamnesis` | P4+P6 | MUST* | Exclusive-run RAG (`evidentia:run:<id>` + `evrun:`) |
| 8 | `openfda` | P4/P7 | MAY 0.5.C/K | FDA label/FAERS/enforcement + ICD-11 |
| 9 | `ema` | P7 | MAY 0.5.C | EU MA + CHMP/EPAR (native; not a gap) |
| 10 | `titck` | P2/P7 | MAY TR | Türkiye drug index (canonical, gated) |
| 11 | `yok-akademik` | P7 KOL | MAY 0.5.E | TR KOL identity (≠ YÖK Tez) |
| 12 | `pophive` | P7 | MAY 0.5.K | **US-only** surveillance |
| 13 | `who-gho` | P7 | MAY 0.5.K | Global/country burden (incl. `TUR`) |
| 14 | `globocan` | P7 | MAY 0.5.K | Cancer incidence/mortality (modelled + `ui`) |
| 15 | `drugddx` | P4 | MAY 0.5.I | Clinical-DDI label pointer (not a pairwise engine) |
| 16 | `med-terminologies` | P1/P4 | MAY coding | ATC / ICD-10→11 / ICD-11 text (D6 ALLOW) |
| 17 | `nih-clinicaltables` | P4 | MAY coding | `drugs` / `icd10cm` **code→desc** / `conditions` |
| 18 | `nlm-rxnorm` | P4 | MAY 0.5.I | `rxnorm_search` / `rxnorm_get_properties` only |
| 19 | `iuphar-gtopdb` | P4 | MAY 0.5.I | Target/ligand when mechanism is in-question |
...
### Companions (9 — not in `.mcp.json`; Cursor may omit them)

| Companion | Home | Duty | Degrade |
|---|---|---|---|
| PubMed (HCLS) | P1.2 | SHOULD | `pubmed-epmc` already covers PubMed/EPMC/Unpaywall |
...
| Paper Search | P1.6 / P4.T2 | SHOULD | EPMC OA + Unpaywall |
| YÖK Tez | P1.7 | SHOULD | TR thesis hole → Türkiye Veri Boşluğu |
| Elicit | P3 | MAY SR-aid | P3 continues native |
| AdisInsight | P2/P7 | MAY 0.5.I | drug-intel appendix omitted |
| Wiley | P4.T4 | SHOULD | OAuth; skip → OpenAthens miss then annas |
| Literatür | P4 | MAY TR | OpenAthens/annas cascade |

Directory-only (not fleet companions — never invent a prefix): **bioRxiv/medRxiv**,
Scholar Gateway, ChEMBL, NPI, Türk Patent, Synapse, OpenTargets. If present this
session, use per CONNECTORS.md §1.0/§1.2; if absent, `SKIP-REASON companion_unloaded`
(preprint hole for bioRxiv) or documented gap.
...
4. **OUT** for all other 18 bundled servers (protocol does not retrieve).
...
| 1.2b | PubMed companion | `search_articles` | SHOULD — else pubmed-epmc stands |
...
| 1.6 | Consensus + Paper Search | Consensus `search` (≤3/batch, usage message verbatim) → Paper Search `search` | SHOULD |
| 1.7 | YÖK Tez companion | `search_yok_tez_detailed` (TR **and** EN) | SHOULD |

**OUT at P1:** `openathens`, `annas-reader`, `anamnesis`, `titck`, `yok-akademik`,
...
2. **MAY:** Elicit `search_papers` / `list_reports` as SR-aid only; extracted claims
   are cross-validated (DEĞİŞMEZ 4). Unloaded → `companion_unloaded`.
...
| T3 | `openathens` | `oa_verify_access` → `oa_fetch_fulltext` (text/RAG) or `oa_fetch_pdf` (original file) | MUST try |
| T4 | Wiley companion | `authenticate` then publisher full-text | SHOULD |
| T5 | `annas-reader` | `article_search`→`read_article`; file: `download_document(id=DOI\|MD5)` | MUST try if T1–T4 failed |
| T6 | `pubmed-epmc` | `pubmed_fetch_fulltext` Unpaywall sweep | MUST try if still empty |

Success at Ti → later tiers `SKIP-REASON already_canonical`. Copyright gate binds every
