...
name: start
description: >
  evidentia süitine giriş ve yönlendirme. 19 bundled MCP + 9 companion bağlanırlığını
  PRISMA faz sırasıyla kontrol eder, flagship medical-research 9.0.2 skill'ini ve yedi
...
`medical-research` **9.0.2**: her derleme **P0–P7** hattından geçer. Filo: **19 bundled
server** (10 gated / 9 public) + **9 companion** (hesap düzeyi; Cursor yüklemeyebilir).
...
2. **P1 companions SHOULD** — PubMed, Clinical Trials, bioRxiv (directory), Consensus,
   Paper Search, YÖK Tez.
3. **P4 cascade** — `openathens` (T3) → Wiley companion (T4) → `annas-reader` (T5) →
   pubmed-epmc Unpaywall (T6) → `anamnesis` (exclusive `evidentia:run:<id>`).
...
6. **İkincil** — Elicit, AdisInsight, Literatür (companion_unloaded dürüst).
...
`medical-research` 9.0.2 ağır işi yapar: P0–P7, native-MCP-first, ordered playbook,
...
| `/evidentia-connectors` | **19+9 preflight** + G-BUNDLE / G-IDENTITY / G-PROBE |
