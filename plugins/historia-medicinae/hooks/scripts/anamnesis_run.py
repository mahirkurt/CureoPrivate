#!/usr/bin/env python3
"""historia-medicinae Anamnesis helpers — exclusive collection + prefixed doc_id.

Contract:
  collection = "histmed:run:<12hex>"
  doc_id     = "hmrun:<12hex>:<canonical>"   (doi/… · wellcome/… · devarsiv/… · yoktez/…)
...
CI: HISTMED_ANAMNESIS_FORGET_LOG stubs HTTP; HISTMED_ANAMNESIS_NO_NETWORK skips
...
NEW_RUN_RE = re.compile(
    r"(?:^|\s)/historia-medicinae(?:\:start)?(?:\s|$)",
    re.IGNORECASE,
)
RUN_ID_RE = re.compile(r"^[0-9a-f]{12}$")
PREFIX_RE = re.compile(r"^hmrun:([0-9a-f]{12}):")
COLLECTION_RE = re.compile(r"^histmed:run:([0-9a-f]{12})$")
...
_INIT = {
    ...
    "params": {
        ...
        "capabilities": {},
        "clientInfo": {"name": "histmed-anamnesis", "version": "1"},
    },
}
...
def scoped_doc_id(run_id: str, raw: str) -> str:
    ...
    if raw.startswith("histmed:run:"):
        rest = raw.split(":", 2)[-1]
        # histmed:run:<id>:<canonical> from another run
        if ":" in rest:
            rest = rest.split(":", 1)[1]
        return want + rest
    if raw.startswith("histmed:"):
        return want + raw[len("histmed:"):]
    return want + raw
...
def ledger_path() -> Path:
    ...
    return Path(tempfile.gettempdir()) / f"anamnesis-histmed-{uid}" / LEDGER_NAME
...
def context_message(ledger: dict) -> str:
    ...
    return (
        "[historia-medicinae] Anamnesis münhasır çalışma seti — "
        f"collection=`{coll}` · doc_id öneki `{prefix}` ({n} kayıtlı belge). "
        "Her ingest: collection + `hmrun:<id>:<kanonik>` (doi/pmid/wellcome/devarsiv/yoktez). "
        ...
        "Stop'ta silinmez. SessionEnd / sonraki `/historia-medicinae` / startup "
    ...
    if base == "ingest_document":
        ...
        if not doc:
            return (
                ...
                f"`{prefix}<kanonik>` (ör. `{prefix}doi/<doi>`)."
            )
