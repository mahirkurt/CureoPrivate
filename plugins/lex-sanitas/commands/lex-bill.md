...
1. **Scope Guard (§6).**
2. **Tam-filo (G0).** `legal-distiller` ile Mod 8 server-listesi. Load-bearing: **tbmm** (yasama tarihçesi) + mevzuat + resmi-gazete + Yarg + yok-akademik + karşılaştırmalı.
...
5. **TBMM 10-adım:** teklif imzaları → ihtiyacı netleştir → Tabaka tespiti → genel gerekçe (10 alt-başlık) → 6-bölüm teklif metni → madde gerekçeleri → İçtüzük Md.81 görüşme-usulü notu → komisyon havalesi (esas = Sağlık/Aile/Çalışma ve Sosyal İşler) → ekler → kalite kapıları.
