...
description: >-
  ...
  hukuk dili → sci-audit (yumuşak delegasyon). Şüphede Scope Guard önceliklidir; bireysel dava (SGK ödeme reddi,
  AYM bireysel başvuru), malpraktis savunması ve promosyon materyali denetimi KAPSAM DIŞIDIR.
version: 3.0.0
---
...
- **Companion (dış connector — bağlıysa tam-filoya dahil; bkz. `/lex-connectors`):** Yargı içtihat (`mcp__Yarg__*`), Open Law (UK+EU, `mcp__Open_Law__*`), Ansvar (300+ reg korpus, `mcp__Ansvar__*`).
...
|---|---|---|
| **G0** | Tam-filo kapsam (§3) | Wire edilmiş 14 MCP + bağlı companion'lar + evidentia (klinik-boyut varsa) + sci-audit **tamamı** ateşlenmiş; her biri kapsam manifestosunda hit/empty/degraded/skipped-with-reason olarak görünür. **Sessiz atlama = FAIL.** |
...
| **G5** | İçtihat + doktrin (R13) | AYM/Danıştay/Yargıtay/AİHM/ABAD zinciri + Türk doktrin taranmış ve atıflı |
| **G6** | Uluslararası kaynak teyidi (R8/10/11/12) | CELEX konsolide doğrulanmış; WHO/ICH/PIC/S/IMDRF güncel; ICESCR 12+AAAQ kontrol |
...
## 5. Yumuşak delegasyon — evidentia + sci-audit

Bu plugin **bağımsızdır**; aşağıdaki iki plugin **varsa** çağrılır (tam-filo ilkesi gereği **her uygun sorguda**), **yoksa** zarifçe degrade eder ("veri boşluğu" işaretiyle, asla uydurmadan) ve bunu manifestoda beyan eder. Tam sözleşme: `../../shared/composition-contract.md`.
