...
Bu dosya, **Lex-Sanitas'ın medical-research orkestrasyon skill'i ile yapısal entegrasyon protokolünü** tanımlar. İki skill arasındaki çift yönlü veri akışı, paralel pipeline tasarımları, kanıt katmanı senkronizasyonu ve composability sözleşmeleri burada operasyonel düzeyde belgelenmiştir.

> **Versiyonlar:** Bu protokol, **medical-research v7.1** + **Lex-Sanitas v2.6.0** birleşim noktasında doğrulanmıştır. medical-research'ün v7.2'ye yükselmesi durumunda Bölüm 9'daki composability sözleşmesi yeniden teyit edilmelidir.

> **Çekirdek prensip:** Lex-Sanitas, **hukuki epistemik otorite**; medical-research, **bilimsel-tıbbi epistemik otoritedir**. İki skill çakışmaz — birbirini tamamlar. Lex-Sanitas çıktısının "bilimsel dayanak", "kanıt katmanı", "ICER analizi", "uluslararası karşılaştırma" boyutları **medical-research tarafından beslenir**. Tersine, medical-research'ün regulatory, HTA ve medical-affairs katmanlarının **Türkiye-spesifik yargı ve mevzuat boyutu Lex-Sanitas tarafından zenginleştirilir**.

---

## 1. Medical-Research Skill'inin Tanınması ve Tetikleme Sözleşmesi

### 1.1. Medical-Research'ün Lex-Sanitas Açısından İşlevi
...
| Çıktı bölümü | İçerik | Lex-Sanitas için kullanım alanı |
|---|---|---|
...
Lex-Sanitas çıktısının kalite eşiğini geçmesi için **aşağıdaki durumlarda medical-research otomatik olarak çağrılır:**

| Lex-Sanitas Modu | medical-research zorunluluk seviyesi | Tetikleyici sinyaller |
|---|---|---|
...
Lex-Sanitas, modunu seçtikten sonra **aşağıdaki kararla** medical-research'ü çağırır:
...
### 2.1. medical-research → lex-sanitas (Downstream Tüketim)

Medical-research, **kanıt katmanını yapılandırılmış formda** Lex-Sanitas'a iletir. İki transfer mekanizması vardır:

**(a) Markdown bölüm transferi:** Medical-research çıktısının § 5, 7, 9, 13, 14, 18, 19 bölümleri Lex-Sanitas tarafından **doğrudan citation-aware** olarak alıntılanır. Atıf formatı:
...
**(b) `.data.json` sidecar transferi:** Medical-research çıktısı eş zamanlı bir `.data.json` sidecar üretir. Bu yapısal veri Lex-Sanitas tarafından **doğrudan tüketilir**. Sidecar şeması:
...
Lex-Sanitas, sidecar tüketim sırasında **her bölümü ilgili moduna eşler:**

| Sidecar bölümü | Lex-Sanitas modu/bölümü | Kullanım |
|---|---|---|
...
### 2.2. lex-sanitas → medical-research (Upstream Sinyal)

Lex-Sanitas, medical-research'e **Türkiye-spesifik mevzuat ve içtihat sinyallerini** iletir. Bu sinyaller medical-research'ün **Türkiye Dörtlüsünü** (TİTCK + SGK + YokTez + AFF:Turkey) zenginleştirir.

**Transfer mekanizması:** Lex-Sanitas, medical-research'ü çağırırken **enriched query** üretir:
...
- § 13.f (TİTCK pozisyonu) **derinleştirilir** — Lex-Sanitas Mevzuat MCP'den gelen primer kaynak metnine atıfla
...
Aşağıda Lex-Sanitas + medical-research birleşik pipeline'ları detaylanmaktadır. Her pipeline, gerçek bir Türk sağlık politika ihtiyacına denk gelir.
...
1. **lex-sanitas** (Mod 8 — TBMM_KANUN_TEKLIFI başlatma)
...
3. **lex-sanitas** (tüketim + Mod 8 protokolü)
...
> ⚠️ **Kapsam dışı:** Dava savunma dilekçesi, Bakanlık avukatlığı defans paketi, bilirkişi sunum deck'i ve somut dava stratejisi lex-sanitas kapsamı DIŞINDADIR (→ saglik-sigorta/ilgili hukuk birimi). Burada yalnızca düzenlemenin **reform/iptal-riski analizi** yapılır.

**Pipeline:**

1. **lex-sanitas** (Mod 3 — ANALYZE)
...
3. **lex-sanitas** (Mod 3/6 — ANALYZE/RIA, iptal riski + reform gerekçe notu)
...
> ⛔ **KAPSAM DIŞI — v2.5.2'de DEPRECATED.** Bu pipeline bireysel SGK ödeme reddi → AYM bireysel başvuru üretimini tanımlıyordu; bu, v2.4 saflaştırma kararı uyarınca lex-sanitas kapsamı DIŞINDADIR ve `saglik-sigorta` / `onko-erisim` skill'lerine aittir. Aşağıdaki adımlar yalnızca **tarihsel kayıt** olarak bırakılmıştır; lex-sanitas tarafından **çalıştırılmaz**. lex-sanitas yalnızca SUT/HTA **geri ödeme politikası reformu** (Mod 1/6/9) bağlamında devreye girer; bireysel başvuru formu veya savunma ÜRETMEZ.
>
...
3. **lex-sanitas** (madde gerekçesi yazımı)
...
1. **lex-sanitas** (Mod 6 — RIA başlatma)
...
3. **lex-sanitas** (DEA + BEF tamamlama)
...
1. **lex-sanitas** (Mod 7 — COMPARATIVE_LAW)
...
> ⛔ **KAPSAM DIŞI — v2.5.3'te DEPRECATED.** Bu pipeline ürün-spesifik/bireysel TİTCK red savunma paketi (Bilimsel Kurul sözlü savunması) üretimine yönelikti; bu lex-sanitas kapsamı DIŞINDADIR (→ onko-erisim/ilgili başvuru sahibi). Aşağıdaki adımlar yalnızca **tarihsel kayıt**; lex-sanitas tarafından **çalıştırılmaz**.
>
...
1. **lex-sanitas** (Mod 3 — ANALYZE, mevcut 16 temel kanun + CBK + ikincil mevzuat envanteri)
2. **medical-research** (paralel)
   - Tüm 9 specialty layer eş zamanlı tetiklenir
   - § 1-20 tam rapor
3. **lex-sanitas** (Mod 8 — TBMM_KANUN_TEKLIFI, 48 ay timeline)
...
## 4. Lex-Sanitas Modlarında Medical-Research Handoff Protokolleri
...
- **Önemli ek:** `promo-censor` skill ile birleşim noktası — medical-research → promo-censor → lex-sanitas
...
**Lex-Sanitas tüketim noktaları (Etki Değerlendirme Raporu — EDR):**
...
## 5. Veri Kalite Sözleşmesi — Lex-Sanitas + Medical-Research

### 5.1. Kanıt Hiyerarşisi Senkronizasyonu

Lex-Sanitas, medical-research'ün **GRADE / Tier 0-6 hiyerarşisini** kabul eder ve hukukî üretiminde aynı çerçeveyi referans alır.

**Operasyonel kural:** Lex-Sanitas DRAFT/OPINE çıktısında **GRADE kanıt seviyesi** belirtilir:
...
**Lex-Sanitas atıf formatı (Türk mevzuat):**
```
1219 sayılı Kanunun 5 inci maddesinin birinci fıkrası
```

**Birleşik çıktıda kullanım:** Medical-research kaynakları Vancouver formatında; Türk mevzuat kaynakları R9 Bölüm 7 + R13 hiyerarşisi uyarınca Lex-Sanitas formatında. **İki format karıştırılmaz; kompleks atıflar Lex-Sanitas öncülü, medical-research zenginleştirici olur.**
...
- **Lex-Sanitas:** "Bu medical-research kanıt zincirine atıfla mevzuat yorumu yapılmıştır; hukuki bağlayıcılık için TBMM Başkanlığı + Bakanlık Hukuk Müşavirliği teyidi gerekir."
...
|---|---|---|
| Klinik etkililik (RCT, SR) | medical-research | "lex-sanitas, klinik kararı vermez" |
| Türk mevzuat metni | Lex-Sanitas (Mevzuat MCP) | "medical-research, Türk mevzuat metnini referans alır ama yorumlamaz" |
| Türk yargı içtihatı | Lex-Sanitas (Hukuki Veritabanları MCP + R13) | "medical-research yargı içtihatı için lex-sanitas'a yönlendirir" |
| FDA/EMA/TİTCK kararları | medical-research § 13 + Lex-Sanitas (Mevzuat MCP) | Ortak yetki — çapraz doğrulama |
| HTA kararları (NICE/ICER) | medical-research § 14 | "lex-sanitas, RIA mod'unda medical-research § 14'e başvurur" |
| Akademik doktrin | medical-research § 5 (YokTez) + Lex-Sanitas R13 | Ortak yetki — Türk doktrin Lex-Sanitas, uluslararası doktrin medical-research |

---

## 6. Operasyonel Uygulama — Adım Adım Lex-Sanitas İş Akışı

### 6.1. Otomatik Tetikleme Karar Ağacı

Lex-Sanitas çağrıldığında, **aşağıdaki karar ağacı** medical-research entegrasyonunu otomatik yönetir:
...
3. Aktif mod ne?
   ...
   │   └── Lex-sanitas_context enriched query üretilir
...
6. Lex-sanitas çıktısı, medical-research kanıt zinciri ile zenginleştirilmiş 
   şekilde üretilir
...
Lex-Sanitas'ın medical-research'e ilettiği zenginleştirilmiş sorgu yapısı:

```yaml
medical_research_call:
  original_user_query: "{{kullanıcı sorgusu}}"
  invoking_skill: "lex-sanitas"
...
| Lex-Sanitas Mod | Zorunlu medical-research § | Opsiyonel § | Sidecar zorunlu mu? |
|---|---|---|---|
...
Lex-Sanitas'a Anti-Pattern listesine **yeni eklenen v2.2 anti-pattern'leri:**
...
**Yasak:** Medical-research çıktısındaki bir kanun maddesi referansını veya AYM bireysel başvuru numarasını **Lex-Sanitas Mevzuat MCP veya Hukuki Veritabanları MCP doğrulaması olmadan** Lex-Sanitas çıktısında kullanmak.

**Doğru yaklaşım:** Medical-research § 13.f (TİTCK pozisyonu) veya § 5 (Türkiye verileri) kullanıldığında, **Lex-Sanitas Mevzuat MCP üzerinden çapraz doğrulama** yapılır.

### 7.3. Skill Yetki Çatışması

**Yasak:** Medical-research'ün yetkili olduğu bir konuda (klinik etkililik, GRADE değerlendirmesi, ICER hesabı) **Lex-Sanitas'ın bağımsız yorum üretmesi**.

**Doğru yaklaşım:** Klinik karar veya HTA değerlendirmesi gerektiren konularda Lex-Sanitas, medical-research'e **delege eder** ve onun çıktısını aktarır; bağımsız klinik yorum üretmez.

### 7.4. Epistemik Etiketsiz Birleşim

**Yasak:** Medical-research çıktısını Lex-Sanitas çıktısına **kaynağı belirtilmeden** entegre etmek.
...
**Yasak:** Medical-research'ten Lex-Sanitas'a veri akışı sağlanıp **upstream sinyalin gönderilmemesi**.

**Doğru yaklaşım:** Lex-Sanitas, medical-research'ü çağırırken **enriched query** üretir; sadece original_query iletmez. Bu, medical-research'ün Türkiye Dörtlüsü ve § 13.f (TİTCK pozisyonu) bölümlerinin derinleşmesini sağlar.
...
Lex-Sanitas + medical-research birleşik çıktıları **iki başlık katmanı** kullanır:

**Üst katman (Lex-Sanitas):** Mevzuat metni, madde gerekçesi, mütalaa, RIA, kanun teklifi
...
[Lex-sanitas üretir — Mevzuat MCP + R13]

## 3. Taslak Metni
[Lex-sanitas üretir — R9 dil katmanı]
...
[Lex-sanitas üretir — Mod 4 21-noktalı denetim]

## 6. Karşılaştırmalı Tablo
[medical-research § 6 + § 13'ten alıntı + lex-sanitas yorumu]
...
[.data.json sidecar — gerekirse Lex-sanitas üretir]
```
...
Aşağıdaki YAML, **lex-sanitas v2.2'nin SMP manifest dosyasına eklenecek** composability bloğudur. Bu, smp-orchestrator skill'inin Lex-Sanitas + medical-research pipeline'larını otomatik öneri yapmasını sağlar.
...
composes_with:
  ...
  - skill: medical-research
    ...
    data_flow:
      lex_sanitas_to_medical_research:
        - primary_legislation_refs
        - yargi_ictihat_focus
        - constitutional_basis
        - international_treaty_refs
        - comparative_law_targets
      medical_research_to_lex_sanitas:
    ...
    quality_contract:
      ...
      citation_format_separation: 
        - turkish_legislation: lex_sanitas_format
...
composes_with:
  ...
  # YENİ — Lex-Sanitas v2.2 bidirectional integration
  - skill: lex-sanitas
    ...
    pipelines_disabled:
      # Cross-skill Scope Guard — lex-sanitas bu hatlar için ASLA çağrılmaz
...
Birleşik bir Lex-Sanitas + medical-research çıktısının kalite kabul kriterleri:
...
☐ 4. İlgili § bölümleri (Bölüm 4'teki tabloya göre) Lex-Sanitas'a tüketildi mi?
...
- Beklenen: P-1 pipeline tetiklenir; medical-research § 1-20 tam çağrılır; lex-sanitas Mod 8 protokolü çalışır
...
- Beklenen: lex-sanitas **AKTİVE OLMAZ**; istem kapsam dışı tespit edilir ve `saglik-sigorta`/`onko-erisim` skill'ine yönlendirilir (SKILL.md §2 Scope Guard). Eski P-3 pipeline **DEPRECATED**'tir.
...
- Beklenen: P-2 pipeline tetiklenir; lex-sanitas Mod 3 (ANALYZE) + Mod 6 (RIA); medical-research 0.5.C + 0.5.E
...
**Lex-Sanitas v2.2 medical-research entegrasyonu — 22 Mayıs 2026**
...
Bu bölüm, medical-research'ün § 1-20 arasındaki tüm çıktı bölümlerini lex-sanitas'ın 9 işletim modunda tam haritalayarak, **hangi medical-research bölümünün hangi lex-sanitas modunda ne tür çıktıya beslendiğini** bire-bir gösterir. v2.2'nin handoff yaklaşımı bölüm bazında genel önerilerle sınırlıydı; v2.3 ile **§ × mod × çıktı kesiti = 180 hücre** matriksi devreye girer.
...
**Tetiklenme mekanigi:** Medical-research'te Adım 0.5 Domain Classifier sözlük tabanlı sinyal tespiti yapar; tetiklenen eksen (A-I) ilgili specialty bölümünü (§ 11-19) çıktıya enjekte eder. Lex-sanitas'tan enriched query üretirken **bu sözlük kelimelerinin doğru tetiklenmesi** kritiktir — bu §19'da operasyonel olarak detaylandırılır.

### 12.2. § × Mod Tam Matrisi (Gerçek § Adlarıyla — v2.3-r1 Kalibre)

Aşağıdaki tablo, **her medical-research § için hangi lex-sanitas modunda zorunlu/önerilen/opsiyonel tüketim** yapılacağını belirtir. Tablo medical-research v7.1'in **gerçek bölüm adlandırması** ile uyumludur:
...
**Lejant:** ◆ = ZORUNLU tüketim (mod ↔ konu klinik bağlamda) — § eksikse Lex-Sanitas çıktısı tamamlanmaz; ▣ = ÖNERİLİR (önemli destek bölümü); ○ = OPSİYONEL (içerik gerektiriyorsa kullanılır); "(clinical mode)" = specialty katman aktif olduğunda zorunlu.
...
### 12.3. Lex-Sanitas Çıktı Bölümü ↔ Medical-Research § Eşleme

Her lex-sanitas çıktı standardı (`SKILL.md Bölüm 8`) bölümü için tipik medical-research § beslemesi:

| Lex-Sanitas Çıktı Bölümü | Tipik Medical-Research § Beslemesi |
|---|---|
...
Lex-Sanitas, medical-research'e enriched query gönderirken **§-bazlı request** ekler. Örnek enriched query:
...
Medical-research v7.1, dokuz specialty layer içerir: Onco, Heme, Regulatory, HTA, MedAffairs, Immunology, Neurology, RareDisease, DrugIntelligence. v2.3, bu layer'ların lex-sanitas'ın 9 modundaki tetikleyici-tabanlı kullanımını matrislemiştir.
...
Sorgu içeriği klinik mi? 
  ├─ Hayır → Specialty layer çağırma; lex-sanitas standalone
...
Birden fazla layer aynı anda tetiklenebilir (cluster). Lex-Sanitas, çoklu tetik durumunda enriched query'de **layer önceliği** belirtir.

---

## 14. Sidecar JSON Şeması v2 (v2.3 YENİ)

Medical-research'ten Lex-Sanitas'a aktarılan `.data.json` sidecar dosyasının **v2.3 zenginleştirilmiş şeması**.
...
### 14.8. Sidecar Tüketim Protokolü — Lex-Sanitas İç Akışı

Lex-Sanitas, sidecar JSON'u tükettikten sonra:

1. **Şema sürümü kontrolü** — `schema_version` ≥ "2.0" olmalı; "2.3" ideal
2. **Sürüm uyumluluk kontrolü** — `medical_research_version` ↔ `lex_sanitas_version` matrisinde (Bölüm 17) kontrol edilir
3. **Veri bütünlüğü kontrolü** — Beklenen bölümler eksik mi? Eksiklik durumunda tekrar query veya graceful degradation
4. **Atıf otomasyonu** — `citation_index` direkt lex-sanitas çıktısının "Kaynakça" bölümüne integrate edilir
5. **Epistemik etiket aktarımı** — `epistemic_honesty.combined_confidence` lex-sanitas çıktısının Bölüm 9 (Bilgi Sınırı) bölümünde sergilenir

---

## 15. Cross-Skill Verification Gates (v2.3 YENİ)

Lex-Sanitas v2.6.0 SMP manifestinde 9 gate (G1-G9), medical-research v7.1 SKILL.md'sinde ~8 gate var. v2.3 ile bu gate'lerin **birlikte değerlendirildiği** çift-skill verification protokolü tanımlanır.

### 15.1. Birleşik Gate Matrisi

| Lex-Sanitas Gate | Medical-Research Karşılığı | Birleşik Test |
|---|---|---|
| **G1** 5210 şekli uyum | (yok — medical-research mevzuat üretmez) | Lex-sanitas tek başına |
| **G2** 5210 maddi-anayasal | M-G3 İçtihat-doktrin temellendirmesi | Lex-sanitas G2 + medical-research § 13 + § 9 entegre kontrol |
| **G3** Türk hukuk dili (R9) | (yok) | Lex-sanitas tek başına |
| **G4** Anti-Pattern (27 madde) | M-G6 Atıf zinciri | Lex-sanitas G4 + medical-research § 20 Vancouver citation cross-check |
| **G5** İçtihat + doktrin (R13) | M-G2 GRADE evidence + M-G5 Tier 0 sentez | Lex-sanitas G5 + medical-research § 7 + § 9 entegre temellendirme |
| **G6** Uluslararası kaynak (R8/10/11/12) | M-G1 Connector kapsama + M-G7 Cross-jurisdiction | Lex-sanitas G6 + medical-research § 6 + § 13.a-d entegre uyum |
...
| **G8** TBMM kapsam (Mod 8 etkin) | (yok — Mod 8 sadece lex-sanitas) | Lex-sanitas tek başına |
| **G9** EX_POST_EVALUATION kapsam denetimi (Mod 9 etkin) | M-G2 GRADE evidence + M-G7 real-world / cross-jurisdiction evidence + M-G8 epistemik şeffaflık | Lex-sanitas G9 + medical-research §5 + §14.f + §19 + M-G8 birlikte PASS olmalı (Mod 9 klinik/HTA içeriyorsa sidecar `ex_post_metrics` zorunlu) |

### 15.2. Birleşik Gate Doğrulama Akışı

```
1. Lex-Sanitas G1-G9 her birini ayrı kontrol et → 9 boolean
2. Medical-Research M-G1-G8 her birini ayrı kontrol et → 8 boolean  
3. Birleşik tabloyu hesapla:
   - G1 (sadece lex-sanitas) PASS gerekli
   - G2 + M-G3 → her ikisi PASS gerekli
   - G3 (sadece lex-sanitas) PASS gerekli
...
4. Genel hüküm:
   ...
   - 1 FAIL (lex-sanitas yanı) → ilgili lex-sanitas remediation
...
- Mod kısıtlaması: COMPARATIVE_LAW modu Singapur HSA dahil edilmedi (lex-sanitas kapsamı dışında veri)
...
> ⚠️ **Kapsam dışı:** Belirli bir firmanın başvuru savunması/regülatör danışmanlığı lex-sanitas kapsamı DIŞINDADIR. Burada yalnızca rejimlerin **karşılaştırmalı mevzuat analizi** yapılır.

**Akış:** medical-research § 13.a (FDA) + § 13.b (EMA) + § 13.c (TİTCK) → lex-sanitas Mod 7 (COMPARATIVE_LAW) — *üçlü mekanik karşılaştırma + Türkiye-spesifik uyum/reform boşluğu* → carbon-html-report
...
**Akış:** medical-research § 13.d (Reliance/WLA) + § 6 (Singapur HSA + Avustralya TGA + Swissmedic Reliance modelleri) → lex-sanitas Mod 7 (COMPARATIVE_LAW) → Mod 1 (DRAFT) — *yeni TR Reliance Yönetmeliği*
...
> ⚠️ **Kapsam dışı:** Belirli bir ürün için bireysel geri ödeme reddi savunması lex-sanitas kapsamı DIŞINDADIR (→ onko-erisim/saglik-sigorta). Burada yalnızca **politika/mevzuat reformu** ele alınır.

**Akış:** medical-research § 7 (Cochrane biyobenzer SR) + § 8 (pivotal biyobenzer trial) + § 13.b (EMA biyobenzer rehberi) + § 14 (HTA biyobenzer fiyat müzakeresi) → lex-sanitas Mod 1 (DRAFT) / Mod 6 (RIA) — *biyobenzer extrapolation/geri ödeme politikası reform metni*
...
**Akış:** medical-research § 13.b (EMA ATMP Regulation 1394/2007) + § 13.a (FDA Cellular & Gene Therapy guidance) + § 9 (ASH/EHA ATMP guidelines) + § 18 (rare disease overlap) → lex-sanitas Mod 1 (DRAFT) — *ATMP Yönetmeliği taslağı*
...
**Akış:** medical-research § 5 (Türkiye RWE — CAR-T sonuçları) + § 14.f (SGK SUT 24 aylık veri) + § 19 (pipeline + competing therapies) → lex-sanitas Mod 9 (EX_POST_EVALUATION) — *Etki Değerlendirme Raporu + revizyon önerisi*
...
**Akış:** medical-research § 5 (Türkiye klinik araştırma istatistikleri) + § 13.b (EU CTR) + § 13.c (TİTCK Klinik Araştırmalar Daire Başkanlığı raporları) + § 6 (karşılaştırmalı AB üye devleti uyumu) → lex-sanitas Mod 9 (EX_POST_EVALUATION)
...
**Akış:** medical-research § 14.a-e (NICE/CADTH/PBAC/IQWiG/HAS karşılaştırma) + § 14.f (SGK SUT entegrasyon) → lex-sanitas Mod 9 (EX_POST_EVALUATION) — *HTA reform ex post*
...
**Akış:** medical-research § 15 (EFPIA/IFPMA/AİFD compliance benchmarks) + § 16 (MedAffairs operations data) + TİTCK Denetim Hizmetleri raporları (web fetch) → lex-sanitas Mod 9 (EX_POST_EVALUATION)
...
Medical-research + lex-sanitas sürüm kombinasyonları için uyumluluk haritası.

### 17.1. Doğrulanmış Kombinasyonlar

| Lex-Sanitas ↓ / Medical-Research → | v6.0 | v7.0 | v7.1 | v7.2 (gelecek) |
|---|---|---|---|---|
...
Lex-sanitas v2.3 + medical-research v7.2 detect (gelecek) →
...
| Lex-sanitas sürüm | Medical-research minimum gerekli | Medical-research önerilen | Notlar |
|---|---|---|---|
...
**Lex-Sanitas için kritik tespit:** Enriched query üretirken bu sözlüklerin **doğru kelimelerini** içermek gerekir. Aksi halde medical-research ilgili specialty katmanını çağırmaz ve lex-sanitas çıktısının çekirdek dayanağı eksik kalır.

### 19.1. Mod-Bazlı Sözlük Tetikleyici Haritası

Aşağıdaki tablo, **her lex-sanitas modu için** medical-research'te hangi domain classifier ekseninin tipik olarak tetiklenmesi gerektiğini gösterir:

| Lex-Sanitas Modu | Tipik Tetiklenmesi Gereken Eksenler | Mod'un Sorgu Bağlamı |
|---|---|---|
...
### 19.2. Eksen-Spesifik Tetik Kelime Listeleri (Lex-Sanitas Enriched Query'sinde Kullanılması Gereken)

#### 19.2.A. Onkoloji ekseni (0.5.A) tetikleme — Lex-Sanitas tarafı
...
> **Operasyonel kural:** Lex-Sanitas Mod 5 OPINE veya Mod 1 DRAFT bağlamında, "akciğer kanseri tedavisinde immünoterapi geri ödemesi" gibi bir Türkçe konuyu enriched query'ye geçirirken **mutlaka** NCCN/ESMO/PFS/OS/ICI gibi metric ve kılavuz adlarını ekleyin. Aksi halde 0.5.A tetiklenmez ve § 11 (onkoloji genişletilmiş) çıkmaz.

#### 19.2.C. Regulatory Sciences ekseni (0.5.C) tetikleme

Bu eksen **lex-sanitas'ın neredeyse tüm modlarında ◆ ZORUNLU** olduğu için enriched query her zaman bu kelimelerle zenginleştirilmelidir:
...
Medical-research v7.1'in 0.5.J kuralı: birden fazla eksen tetiklenebilir (cluster). Lex-Sanitas tarafında **5 primary use case cluster** §13.2'de tanımlanmıştı. Bu cluster'lar 0.5.J ile uyumludur:

| Cluster (R14 §13.2) | Tetiklenen Eksenler (0.5.x) | Tipik Lex-Sanitas Modu |
|---|---|---|
...
Lex-Sanitas, medical-research'e enriched query göndermeden önce şu adımları uygular:

```
1. Kullanıcı sorgusunu Türkçe-İngilizce çift dilde paraleli tara
2. Mevcut Lex-Sanitas modunu tespit et (Mod 1-9)
...
## 20. Reverse Channel — Medical-Research'ten Lex-Sanitas'a Feedback Sözleşmesi (v2.3-r1 YENİ)

R14 v2.2-v2.3 boyunca veri akışı **lex-sanitas → medical-research → lex-sanitas** şeklindeydi: lex-sanitas enriched query gönderir, medical-research çıktı + sidecar üretir, lex-sanitas tüketir. v2.3-r1 ile **medical-research'ün çıktıya iliştirdiği geri bildirim sinyalleri** sözleşmesi tanımlanır.
...
- **Kapsam dışılık:** "Bu sorgu medical-research'ün kapsamının dışında; lex-sanitas için bağlamsal anlam yok"
...
Bu sinyaller lex-sanitas'ın çıktısının kalitesini etkiler. **Reverse channel** olmadan lex-sanitas bu nüansları kaybeder.
    ...
    "uncertainty_flags": [
      {
        ...
        "impact_on_lex_sanitas": "Mod 9 ex post için 12 aylık veri eksik; 2024 baz alındı not edildi"
      },
      {
        "section": "§13.c TİTCK Pozisyonu",
        "type": "single_source",
        "description": "TİTCK ruhsat tarihi sadece bir kaynaktan teyit edildi (resmî portal)",
        "impact_on_lex_sanitas": "Kritik karar verirken ek doğrulama önerilir"
      }
    ],
    ...
    "out_of_scope_flags": [
      {
        ...
        "recommendation_for_lex_sanitas": "Bu bölüm için lex-mercator veya Hukuki Veritabanları MCP doğrudan çağrılmalı"
      }
    ],
...
### 20.3. Lex-Sanitas'ın Reverse Signal İşleme Protokolü

```
1. Sidecar JSON yüklendiğinde, `reverse_signals` alanını ÖNCELİKLE oku
2. `uncertainty_flags` her birini:
   - Lex-sanitas çıktısının ilgili bölümüne FOOTNOTE olarak ekle
...
5. `alternative_interpretations` her birini:
   - Lex-sanitas çıktısının Yönetici Özeti bölümünde "Bu çıktı [seçim] 
...
> **G-Reverse:** Sidecar JSON'da `reverse_signals` mevcut + boş olmayan herhangi bir alt-alan (uncertainty_flags veya out_of_scope_flags veya retry_triggers) varsa, lex-sanitas çıktısının Yönetici Özeti veya Bölüm 9 (Bilgi Sınırı) bölümünde **görünür biçimde** sergilenmiş mi? PASS/FAIL.

### 20.5. Reverse Channel — Pratik Faydaları

- **Şeffaflık:** Lex-sanitas kullanıcısı, medical-research'ün belirsizliklerini doğrudan görür
- **Yetki sınırı netliği:** Hangi bölüm medical-research yetkisinde, hangi bölüm lex-sanitas yetkisinde net olur
...
| **§20 (YENİ v2.3-r1)** | **Reverse Channel — Medical-Research'ten Lex-Sanitas'a Feedback Sözleşmesi** | **~150** |
...
- Promo-censor skill ile üçlü entegrasyon (medical-research § 15 + lex-sanitas Mod 4 + promo-censor) — materyal PASS/FAIL hükmü promo-censor'a aittir; lex-sanitas yalnızca tanıtım mevzuatı reform metni girdisi sağlar
