# Medical-Research Skill ile Operasyonel Entegrasyon — Lex-Sanitas Yan Sözleşme

Bu dosya, **medical-research v7.1 skill'inin composition-runbook.md §10 (Data Transfer Protocols)** bölümünde lex-sanitas için açık bir alt-bölüm **bulunmadığı** için, lex-sanitas tarafından yazılan **karşı sözleşmedir**. Medical-research'ün composition-runbook'unda:
...
- §10.5 medical-research → **lex-sanitas** ❌ tanımlanmamış

Bu dosya, **§10.5'in lex-sanitas tarafından yazılmış karşılığı** olarak işler. R14 ana sözleşmesi ile birlikte okunmalıdır.
...
### 1.1. Medical-Research → Lex-Sanitas Transfer Object
...
### 1.2. Lex-Sanitas Beklentisi

Medical-research çıktısından lex-sanitas şunları tüketir:
...
## 2. Sidecar Consumption Pseudocode (Lex-Sanitas Tarafı)

Aşağıdaki pseudocode, lex-sanitas'ın medical-research'ten gelen sidecar JSON'u nasıl tükettiğini gösterir:

```python
# Lex-Sanitas çıktı üretim akışı (pseudocode)

def consume_medical_research_output(markdown: str, sidecar_json: dict) -> LexSanitasOutput:
    """Medical-research çıktısını lex-sanitas çıktısına dönüştürür"""
    ...
    if schema_version not in ["2.0", "2.3", "2.3.1"]:
        raise SchemaIncompatibilityError(
            ...
            f"lex-sanitas v2.6.0 requires medical-research v2.3+"
        )
    
    # 2. Sürüm uyumluluk kontrolü (R14 §17)
    mr_version = sidecar_json.get("medical_research_version")
    lex_version = sidecar_json.get("lex_sanitas_version")
    ...
    # 6. Lex-sanitas modu spesifik tüketim haritası
    mode = get_current_lex_sanitas_mode()  # Mod 1-9
    ...
    confidence = compute_combined_confidence(
        ...
        lex_disclosure=epistemic.get("lex_sanitas_self_disclosure")
    )
    ...
    return assemble_lex_sanitas_output(
        markdown=markdown,
...
def handle_reverse_signals(reverse_signals: dict) -> None:
    ...
    for flag in reverse_signals.get("uncertainty_flags", []):
        add_footnote_to_section(
            ...
            impact=flag["impact_on_lex_sanitas"]
        )
    ...
    for flag in reverse_signals.get("out_of_scope_flags", []):
        add_user_recommendation(
            ...
            f"Önerilen ek skill: {flag['recommendation_for_lex_sanitas']}"
        )
...
Bu bölüm, **gerçek bir senaryoda** lex-sanitas + medical-research entegrasyonunun adım adım nasıl işlediğini gösterir.
...
### 3.2. Adım 1: Lex-Sanitas Mod Tespiti

Lex-Sanitas trigger analysis: **Mod 1 (DRAFT)** — yeni yönetmelik taslağı.
...
Lex-Sanitas medical-research'e şu enriched query'yi gönderir:
    ...
    "uncertainty_flags": [
      {
        ...
        "impact_on_lex_sanitas": "Madde gerekçesinde 'Türkiye yerel verisi henüz mevcut değil; uluslararası KEYNOTE-A18 verisi temel alındı' notu eklenmeli"
      }
    ],
...
### 3.6. Adım 5: Lex-Sanitas Çıktı Üretimi

Lex-Sanitas, sidecar JSON ve markdown çıktısını tüketerek **Yönetmelik Taslağı + Genel Gerekçe + Madde Gerekçeleri** üretir.
...
| Senaryo | Lex-Sanitas Modu | Tetiklenen Eksenler | Çıktı Tipi |
|---|---|---|---|
...
```markdown
### 10.5 medical-research → lex-sanitas

**Transfer object:** Markdown §1-§20 + sidecar JSON v2.3.1.

**lex-sanitas beklenti:**
- §1 Küresel Literatür (yönetici özeti için)
...
**Lex-Sanitas tüketim protokolü:**
- Şema sürümü ≥ 2.3.1 kontrolü
- Reverse signals önceliği (R14 §20)
- Mod-bazlı tüketim haritası (Lex-Sanitas R14 §12.2 matrix)
- Cross-skill verification (Lex-Sanitas R14 §15)
- Çift epistemik dürüstlük etiket sergileme

**İletişim:** Lex-Sanitas v2.6.0 ile doğrulanmış; v2.3.1+ sidecar şemasıyla geriye uyumludur (Lex-Sanitas R14 §17 
...
1. **Klinik olmayan konu (örn. tütün düzenlemesi):** Medical-research çağrılmamalı; Lex-Sanitas standalone çalışır
...
| Durum | Davranış |
|---|---|
| Medical-research v7.0 algılandı (Reverse signals yok) | Lex-Sanitas devam eder; sidecar JSON v2.0 ile çalışır; uyarı: "v7.1+ önerilir" |
...
| AdisInsight MCP unavailable | §19 boş geçer; Lex-Sanitas DrugIntel sec'i `[veri eksik — AdisInsight unavailable]` ile işaretler |
