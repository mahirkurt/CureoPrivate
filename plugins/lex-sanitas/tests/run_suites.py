...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="lex-sanitas davranış süiti koşucusu")
    ...
    for nm, fn in (("ledger_fixture", "evidence_ledger.schema.json"),
        ...
        p = ROOT / "skills" / "lex-sanitas" / "schemas" / fn
        if p.is_file():
    ...
    for sf in suites:
        ...
        for c in cases:
            ...
            for ref in set(re.findall(r"`((?:references|templates|schemas|shared|hooks)/[^`]+?\.(?:md|json|yaml|py))`", txt)):
                cand = [ROOT / "skills" / "lex-sanitas" / ref, ROOT / ref]
