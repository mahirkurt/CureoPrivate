# paideia — Paylaşılan Connector Sözleşmesi (CONNECTORS.md)

**Belge sınıfı:** Normatif connector envanteri — plugin-düzeyi tek doğruluk kaynağı
**Sürüm:** 1.0.0
**Kapsam:** `paideia` plugin'inin flagship `carbon-paideia` skill'inin (ve ileride
...
> **Neden bu dosya var.** Standalone `carbon-paideia` skill'inde connector envanteri +
...
Bu uyarılar `carbon-paideia/references/curriculum-integration.md §2`'den taşınmıştır ve
**normatiftir**:
...
Bu politika `carbon-paideia`'nın görsel üretim yolunu plugin düzeyinde sabitler. Skill'in
...
- `curriculum` bloğu (`MODULE_DATA` uzantısı) her kazanım için `code` + `text` + `skill` (KB2.x)
  ...
  `carbon-paideia/references/curriculum-integration.md §5`.
...
Bu, korpus sürümünü de verir (provenans damgası). `/paideia:durum` komutu bu pre-flight'ı
...
Bu connector sözleşmesi ileride `paideia` plugin'ine eklenecek **sibling skill'ler** tarafından
paylaşılabilir:
...
`carbon-paideia` + `start` paketlenmiştir.
