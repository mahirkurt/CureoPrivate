...
`maarif-mufredat` connector'ının sağlık kontrolünü yap. Bu, `paideia:start` skill'inin Adım 2'sini
...
1. **Pre-flight / canlılık** (`../CONNECTORS.md §6`): `server_info` çağır. Yanıt verirse
   ...
   Settings → Connectors'tan `maarif-mufredat`'ı etkinleştirmesini söyle ve `carbon-paideia`'nın
