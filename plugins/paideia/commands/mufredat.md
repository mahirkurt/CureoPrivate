...
`paideia:carbon-paideia` skill'ini **CURRICULUM modunda** çağır. Mantığı tekrarlama — bu komut
ders+sınıf+konu girdisini kanonik kazanım keşfine ve `/paideia:modul` üretim akışına bağlar.
...
4. **`/paideia:modul` akışının 2–4. adımlarını uygula:** beceri → etkileşim haritalama
...
Tek bir kazanım kodu verildiyse → `/paideia:modul` (doğrulama+çekme kısayolu). Yalnız keşif
(üretim yok) isteniyorsa → `/paideia:kazanim-bul`. Ders Türkiye MEB dışıysa (IB/Cambridge) →
