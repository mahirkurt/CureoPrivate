...
# Carbon Paideia — DEHB-Odaklı Etkileşimli Öğrenim Modülü Üreticisi
...
- `figma-forge` — paideia token'larını (`carbon-v11-authority.json`) Figma
...
**Orkestrasyon:** `smp-orchestrator pipelines carbon-paideia` geçerli hatları
listeler (örn. `francais-coach → carbon-paideia`,
`carbon-paideia → carbon-html-report → pdf`).
