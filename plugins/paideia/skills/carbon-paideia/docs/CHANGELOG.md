# Değişiklik Günlüğü — carbon-paideia

Bu proje [Semantik Sürümleme](https://semver.org/lang/tr/) kullanır.

## [2.8.0] — 2026-07-06

### paideia PLUGIN ENTEGRASYONU

Standalone `carbon-paideia` skill'i, `paideia` plugin'inin flagship skill'i olarak paketlendi.
...
- **Beceri → etkileşim deseni haritalama tablosu** (çekirdek katma değer) — Maarif
  ...
  `carbon-paideia` etkileşim desenlerine yapıcı-hizalama (constructive alignment)
...
- Üç yeni demo: `carbon-paideia-fen-demo.html`, `carbon-paideia-sosyal-demo.html`,
  `carbon-paideia-dil-demo.html`.
...
- `meta.subject`/`meta.subjectKey` → **derse-özel kimlik aksanı** (math mor, fen teal, …) +
  ...
  Carbon değer tablosu. Yeni **matematik demosu** (`carbon-paideia-math-demo.html`).
...
- Varsayılan örnek modüle **flashcards** segmenti eklendi; tüm yeni türler için
  ayrı **vitrin demosu** (`carbon-paideia-showcase.html`).
