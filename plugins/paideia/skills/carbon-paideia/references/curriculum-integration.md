...
> Bu referans, `carbon-paideia`'nın **Müfredat MCP** sunucusunu kullanarak
...
> **Plugin-entegrasyon notu (paideia).** Bu skill `paideia` plugin'i altında paketlendiğinde,
...
> Bu alt-bölüm, `paideia` plugin sarmalayıcısının görüntü-dayanak sözleşmesini bu skill'de
...
**Adım 6 — Modülü kur, doğrula, sun.** Normal `carbon-paideia` iş akışına dön
...
`carbon-paideia`'nın etkileşim desenleriyle doğrudan örtüşür. Kazanım üst-fiilini
...
- **Yaş uygunluğu korunur.** MCP yalnız MEB içeriği verir; yine de hedef sınıf
  düzeyine sadeleştirme ve `carbon-paideia` çocuk-güvenliği kuralları geçerlidir.
...
> bağımlılık değil. `carbon-paideia`'nın çekirdek değeri (kaynaktan etkileşimli,
