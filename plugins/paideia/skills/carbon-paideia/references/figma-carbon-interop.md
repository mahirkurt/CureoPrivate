...
> Bu referans, `carbon-paideia` token otorite zincirinin **üçüncü (opsiyonel)
...
6. `figma-forge` composability — paideia ekranlarını Figma'ya itmek
...
- Kullanıcı bir **Figma dosya URL'si** paylaştığında ve paideia çıktısının o
...
Ters yön (paideia → Figma) bu skill'in kapsamı dışındadır; **`figma-forge`**
skill'i ile yapılır:

- `figma-forge`'un **Carbon v11 yerleşik mapper'ı** `--cds-*` token'larını
  Figma Variables'a, paideia bileşen desenlerini (stepper, tile, notification,
  tag) variant ComponentSet'lere dönüştürür.
- Akış: paideia modülü üret → `figma-forge` `TOKENS_IMPORT` modu ile
...
- Figma kütüphanesindeki tema modları (White/G10/G90/G100) 4'lüdür; paideia
