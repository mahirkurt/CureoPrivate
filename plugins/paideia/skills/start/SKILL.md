---
name: start
description: paideia süitine giriş ve yönlendirme. Bağlı Maarif Modeli MCP connector'ını (maarif-mufredat) kontrol eder, flagship carbon-paideia skill'ini ve dört komutu tanıtır, kullanıcının niyetine göre doğru komuta yönlendirir. İlk kez süitle çalışırken, hangi connector'ın bağlı olduğunu görmek için, ya da "paideia nedir / nereden başlamalıyım / hangi komutu kullanmalıyım / connector'ım bağlı mı / Maarif MCP çalışıyor mu" türü oryantasyon sorularında kullanın. Tetikleyiciler — paideia başlat, süit oryantasyonu, connector kontrolü, "ne yapabilirsin", "nereden başlayayım", "Maarif Modeli modülü nasıl üretirim", "kazanımdan modül nasıl".
version: 1.0.0
last_updated: 2026-07-06
---

# paideia — Başlangıç ve Yönlendirme

paideia, **MEB Türkiye Yüzyılı Maarif Modeli kazanımlarından DEHB-dostu, erişilebilir,
...
```
paideia — MEB Maarif Modeli Etkileşimli Öğrenim Modülü Süiti
...
`carbon-paideia`'nın **MCP olmadan da** (kullanıcının verdiği ders metniyle) çalıştığını, üretimin
...
| Bileşen | Ne Yapar |
|---|---|
| **carbon-paideia** (flagship skill) | 8 mod, 11 kalite kapısı — kaynaktan/kazanımdan tek-dosya etkileşimli HTML öğrenim modülü |
...
|---|---|---|
| `/paideia:modul` | Kazanım kodundan modül üretir | `<kazanım-kodu>` (örn. FB.5.3.1.1) |
| `/paideia:mufredat` | Ders+sınıf+konudan modül üretir | `<ders> <sınıf> <konu>` (örn. Fen 5 hücre) |
| `/paideia:kazanim-bul` | Konu→kazanım keşfi + KB/etkileşim haritası (**üretim yok**) | `<konu> [sınıf] [ders]` |
| `/paideia:durum` | Connector sağlık + Tier-2 (get_figure) kontrolü | — |

## Adım 5 — Niyete Göre Yönlendir

Kullanıcının ne üzerinde çalıştığını sorun ve yönlendirin:

1. **Kazanım kodu verildi** (`FB.5.3.1.1`) → `/paideia:modul`.
2. **Ders + sınıf + konu** ("5. sınıf fen hücre") → `/paideia:mufredat`.
3. **"Bu konuya hangi kazanımlar denk geliyor?"** (yalnız keşif) → `/paideia:kazanim-bul`.
4. **Connector çalışıyor mu / Tier-2 var mı?** → `/paideia:durum`.
5. **Kaynak metin yapıştırıldı** (MCP yok) → `carbon-paideia` skill'ini **doğrudan** (MCP'siz)
