...
`output-templates.md` / `osint-playbook.md` / `execution-map.md` / `composition-runbook.md` /
...
| `Exa site:titck.gov.tr {konu}` | `TİTCK:search_drugs/get_atc_class_summary/...` (native — `turkiye-layer.md`) |
| `Exa site:sgk.gov.tr OR resmigazete.gov.tr` | `Mevzuat:search_mevzuat/get_mevzuat_text` (native) |
...
| Tavily absent | Add `Tavily:tavily_search` (Exa-fallback, quota-aware) where web research is used |
...
- Tier 6 OSINT note: AdisInsight `history_events` (real) is curated intel (Tier 6 context), never clinical-claim support.

## D. `output-templates.md` (update)
- Sidecar bump `medical_research_sidecar_version: "8.0"`.
- `connectors_used` → replace with verified list: `["PubMed/EuropePMC(8f314cbe)","Consensus","ScholarGateway","PaperSearch(660e91bd)","ClinicalTrials(4cc36ce0)","bioRxiv","YÖKTez","Exa","Tavily","AdisInsight(6a9fd4a4)","ChEMBL(bio-research)","RegulatoryMCP(922d7cdc)","TİTCK(1a49b1bb)","Mevzuat(fbf16a1a)","TÜRKPATENT(ded65854)","NPI(64557ced)","annas-mcp","Synapse(auth)","Wiley(auth)"]`.
...
| File | v8.0 insert |
|---|---|
| `oncology-layer.md` | TİTCK `find_off_label_uses_for_drug` (native onko off-label) replaces Exa SGK onko scraping; AdisInsight real `search_drugs(mechanism=..., targets=..., therapeutic_area="Cancer")` for ADC/bispecific/CAR-T landscape; GLOBOCAN via `regulatory-intelligence` 0.5.K; OncoKB/CIViC via Exa retained |
| `hematology-layer.md` | AdisInsight real schema for CAR-T/bsAb/ADC landscape; TİTCK native for TR heme reimbursement; ASH/EHA abstract via Exa retained; full-text via `fulltext-retrieval.md` |
| `regulatory-science-layer.md` | native `openfda_search(drug/drugsfda, drug/label, drug/enforcement)` + ICD-11 + AdisInsight `history_events` (CRL/ODAC/registration) + EMA(Exa) + TİTCK native; withdrawal pool = enforcement + AdisInsight discontinued |
| `hta-layer.md` | WHO GHO burden (0.5.K) for budget-impact; TİTCK price + Mevzuat SUT (native) for §14.f; NICE/CADTH/IQWiG/HAS via Exa retained; AdisInsight competitor set (real schema) for comparator |
| `medaffairs-ops-layer.md` | NPI `npi_search` for ABD KOL/PI verification; AdisInsight `conference_coverage` (real) + OpenAlex KOL graph; EFPIA/IFPMA/İEİS via Exa retained |
| `immunology-layer.md` | AdisInsight real schema for JAK/TYK2/IL-17/23 landscape (`mechanism=...`); TİTCK native for TR biologic reimbursement; ACR/EULAR via Exa retained |
| `neurology-layer.md` | AdisInsight real schema (anti-amyloid/gene therapy/CGRP); TİTCK native for TR MS/SMA reimbursement; Synapse AD Knowledge Portal (auth, conditional); AAN/ECTRIMS via Exa retained |
| `rare-disease-layer.md` | Orphanet/OMIM via Exa retained; WHO GHO + ICD-11 (0.5.K) for prevalence; AdisInsight orphan flags (`is_orphan_drug`, real) ; TİTCK Madde-23/import (native); Synapse natural-history (auth, conditional) |

## F. Infrastructure files
- `osint-playbook.md` — patent playbook adds **TÜRKPATENT** (`search_patents`) for TR IP; pharmacovigilance playbook uses native `openfda_search`; competitive uses AdisInsight real schema + Tavily(Exa-fallback).
- `execution-map.md` — add native-first batching: native MCP calls parallel-safe **except** regulatory MCP (single + retry) and Tavily (try→Exa fallback). Update wallclock with latency caveat.
