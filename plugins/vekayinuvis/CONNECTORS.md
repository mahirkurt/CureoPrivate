...
| `annas-reader` *(tam-metin)* | http | `https://annas.cureonics.com/mcp` | **Son-çare kitap+makale tam-metin**, 9 araç (2026-08-14). Bounded reader akışı + `download_document(id=DOI\|32-hex MD5)` ile PDF/EPUB/MOBI/AZW/DjVu/FB2/CBZ/CBR/XPS kısa-ömürlü resource link + SHA-256. DOI isteği Crossref/içerik-kimliği kapısından geçer. Telif: yalnız analiz | HP self-host (Docker), `/mcp` Bearer (`${ANNAS_MCP_API_KEY}`) |
