...
CONVENTIONS = (
    ...
    "(300 DPI görüntü, asistan görüsüyle okur); viewer temsilî-sayfa OCR/HTR "
    "(`devarsiv_ocr_belge_pages`) ikincil kanıttır, TAM yol yerel arşivdir. "
    "(2d) K4 SYNC/ASYNC — ≤5 sayfa + tek motor → `devarsiv_ocr_archive_pages` (sync); >5 sayfa "
    "veya `both` tam belge → `devarsiv_ocr_submit` → `devarsiv_ocr_result(include_text=false)` ile "
