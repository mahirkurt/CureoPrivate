...
CONVENTIONS = (
    ...
    "oa_fetch_pdf) → `annas-reader` (Tier 4 son çare: bounded reader; orijinal PDF/EPUB/etc. "
    "download_document; yalnız analiz; `marmara` Tier 3b wire bekliyor — DNS yayınlanmadı). "
    "Kısa-ömürlü resource_link derhal tüketilir; DOI/MD5 + SHA-256/provenance saklanır. "
    "Getirilen tam-metin > eşik → anamnesis ingest "
    "(collection=`vekayinuvis:run:<12hex>` + doc_id `vkrun:<12hex>:<kanonik>`; "
    "kapsamsız hybrid_query/graph DENY; atıf `doc_id::idx`; Stop'ta silinmez). "
    ...
    "alınmamış da okunur; önizleme taramasındaki '…görüntülenmiştir' filigranı `devarsiv_ocr_belge`'de "
    "Katman-0'da bastırılır, satın-alınmışsa `purchased_hint` temiz arşiv sayfalarına yönlendirir) ve "
    "`devarsiv_ocr_belge` ile metne çevrilir: Latin/Cumhuriyet tam-metin, "
    ...
    "sepetini değiştirir ama asla otonom ödeme yapmaz. `add_to_cart`'ta `hash` OPSİYONEL "
    "(verilmezse sunucu store/canlı aramadan çözer, asla uydurmaz); belge zaten satın-alınmışsa "
    "`already_purchased`+`next_steps` döner. Yanıt slim (<20 KB). "
    ...
    "(${CLAUDE_PLUGIN_ROOT}/shared/coverage-manifest.md: her server için hit/empty/degraded/skipped-with-reason). Ham veri "
    ...
    "${CLAUDE_PLUGIN_ROOT}/shared/context-economy-contract.md. Bir katman yoksa alt katmana degrade + manifestoda beyan "
