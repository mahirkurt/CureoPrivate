...
| Belge | `devarsiv_get_belge(item_id, hash, arsiv)` | Künye + `access` (**purchased**=SatinAldiklarim ledger-doğrulanmış · **preview**=önizleme ibaresi var ama sahip DEĞİL · purchasable) + `purchased` bool |
| Belge | `devarsiv_get_belge_image(item_id, hash, arsiv)` | Önizleme taraması ImageContent — **görüyle okuma** |
| Belge | `devarsiv_ocr_belge(item_id, hash, arsiv, lang?, engine?)` | OCR/HTR; Osmanlı varsayılanı `engine="transleyt"`; önizleme filigranı Katman-0'da bastırılır; satın-alınmışsa `purchased_hint` temiz sayfalara yönlendirir |
...
| Sepet | `devarsiv_add_to_cart(item_id, hash?, arsiv, pages?)` `[_RW]` | 1-tabanlı cbk sayfa seçimi ("1,3-5"; boş=tümü). **`hash` opsiyonel** (store→canlı arama çözer, uydurulmaz); satın-alınmışsa `already_purchased`+`next_steps`; yanıt slim (<20 KB) |
...
| Arşiv | `devarsiv_rebuild_archive(incremental?, limit?)` `[_RW]` | Satın-alınanları yerel 300 DPI PDF arşivine kurar/günceller (eSatış ZIP→kayıpsız çok-sayfa PDF); **TAM-belge tek yolu** (viewer temsilî-tek-sayfa sınırını aşar); `session_required` korumalı, ödeme YAPMAZ; `incremental=True` mevcutu korur (temiz blok yeniden İNMEZ). **Her talep BENZERSİZ blok** (sayfa-aralığından deterministik kod `{fon}.{no}.p{ilk:03d}` → çakışma/kopya YOK, front talepleri sayfalanmış listeden okunur). Sonuç: `{status(ok\|integrity_error), added, kept, renamed, failed:[{t,code,yer,error}], count, integrity:{violations, duplicate_blocks, missing_pages}}`. **`status:integrity_error`** = bir belgenin kapsamında delik/kopya var (rapor `integrity`'de) → o belge OCR'ı güvenilmez, yeniden koş |
...
| **OpenAthens** *(tam-metin Tier 3)* | `openathens` — metin/RAG: `oa_resolve`→`oa_fetch_fulltext`; orijinal provider PDF: `oa_fetch_pdf(doi\|url)`; ayrıca `oa_verify_access`, batch/search/fetch | **Lisanslı kitap+makale tam-metni** — Millet Kütüphanesi/OpenAthens SAML; provider-nötr orijinal PDF kısa-ömürlü resource link + SHA-256/provenance ile | Kitap/makale tam-metni gerektiğinde (literatur/paper-search'ten SONRA) |
| **Anna's Reader** *(tam-metin Tier 4, son çare)* | `annas-reader` — bounded reader: `book_search`, `article_search`, `get_document_info`, `read_document`, `read_article`, `search_in_document`; orijinal dosya: `download_document(id=DOI\|32-hex MD5)` | **Son-çare kitap+makale tam-metni** — PDF/EPUB/MOBI/AZW/DjVu/FB2/CBZ/CBR/XPS kısa-ömürlü resource link + checksum; telif: yalnız analiz | Lisanslı band getiremeyince (openathens'ten SONRA) |
...
**`openathens`** (lisanslı — metin/RAG `oa_resolve`→`oa_fetch_fulltext`; orijinal
provider PDF `oa_fetch_pdf(doi|url)`) → **`annas-reader`** (son çare — bounded reader
akışı; orijinal PDF/EPUB/etc. `download_document(id=DOI|MD5)`; yalnız analiz).
Dosya araçlarının opaque `resource_link`'i kısa ömürlüdür: derhal tüketilir,
kalıcı kaynak diye cache'lenmez; DOI/MD5 + format + SHA-256/provenance saklanır ve uzun
dosya anamnesis'e ingest edilerek bounded sorgulanır.
